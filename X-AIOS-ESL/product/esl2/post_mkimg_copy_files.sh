...
if [ ! -f "$MC_IMAGE_BUILDER" ]; then
    MC_IMAGE_BUILDER="$PRJ_TOP_DIR/base/soc/qm10xd/linux/bsp/uboot/uboot/tools/mc-image-builder"
fi

SRC_OUT_BSP_DIR=${QM_ALL_IMAGES}/../image/bsp

_soc_pkg_path=base/soc/qm10xd/linux
PREBUILT_OUT_BSP_DIR=${_soc_pkg_path}/prebuilts/esl2/bsp
...
if [[ "$CONFIG_STORAGE_TYPE_EMMC" = true ]]; then
	...
	#cp ./swap_2KB_128KB_10MB.ubi $QM_ALL_IMAGES
	...
	cp $PREBUILT_OUT_BSP_DIR/recovery/$CONFIG_PLATFORM_ID/recovery.img $QM_ALL_IMAGES
	cp $PREBUILT_OUT_BSP_DIR/recovery/$CONFIG_PLATFORM_ID/recovery-rootfs_2KB_128KB_*.ubi $QM_ALL_IMAGES
	...
	cp $PRJ_TOP_DIR/tools/partition/$CONFIG_XOS_BUILD_BOARD/los_spi-nand/64M/$CONFIG_XOS_PROJECT_TYPE/programmer.ini $PROGRAMMER_DIR
        ...
        $MC_IMAGE_BUILDER -c  8/1024   -p 2048  -o 64  -e 0x20000  -n 64  $QM_ALL_IMAGES/system_2KB_128KB_18MB.ubi $PROGRAMMER_DIR/system_2KB_128KB_18MB.ubi.burn
...
echo "QM: copy boot QM_VERSION:$QM_VERSION.$BUILD_NUMBER HW:$QM_HW_VERSION"
...
cp $QM_ALL_IMAGES/system_2KB_128KB_18MB.ubi $QM_ALL_IMAGES/update/
...
#cp $QM_ALL_IMAGES/data_2KB_128KB_77MB.ubi $QM_ALL_IMAGES/update/
...
echo $CONFIG_PLATFORM_ID > version.txt
# u-boot-spl-header.img
tar -czvf update-$CONFIG_QM_VERSION.$BUILD_ID-$CONFIG_QM_HW_VERSION.tar.gz *
mv update-$CONFIG_QM_VERSION.$BUILD_ID-$CONFIG_QM_HW_VERSION.tar.gz $QM_ALL_IMAGES/
cd -
rm -rf $QM_ALL_IMAGES/update/
