...
if [ $loop -le 10 ]; then
    ...
    while [ $ATTEMPT -le $MAX_RETRY ]; do
        ...
        # /data/update.tar.gz
        downloader_esl "$current_version" "$hardware_version" --check_update --addr $service_addr
        RET=$?
        case $RET in
            0)  # 
                echo "has_new = true"
    ...
    if [ $final_ret -eq 0 ]; then
        ...
        # 如果/data/update.ini存在,就从文件里面解析出url地址, 文件格式是:
        # ~#cat data/update.ini
        # update_version=V2.00.03-J32
        # update_url=https://kiwp.oss-cn-beiiing.aliyuncs.com/update-V2.00.03.tar.gz
        # update_md5=6cb02ec9858a16b674e5321e4aa3be22
        # 如果/data/update.ini不存在,或者文件没有update_url字段,就直接exit 1
        # 如果/data/update.ini存在,并且有update_url字段,就进入boot2Recovery
        if [ -f /data/update.ini ]; then
            update_url=$(grep -E '^update_url=' /data/update.ini | cut -d '=' -f 2)
            if [ -n "$update_url" ]; then
                echo "Downloading update.tar.gz from $update_url"
            else
                echo "update_url not found in /data/update.ini, exiting"
                exit 1
            fi
        else
            echo "/data/update.ini not found, exiting"
            exit 1
        fi
    fi
else
