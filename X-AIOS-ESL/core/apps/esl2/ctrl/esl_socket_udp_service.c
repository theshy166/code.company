...
void send_broadcast_message(char * broadcast_ip,char * msg) {
    QM_ESL2_LOG("Broadcast message sent: %s , %s\n",broadcast_ip, msg);
    int sockfd;
    ...
    // 发送广播消息
    QM_ESL2_LOG("send_broadcast_message sin_port: %d\n",OPEN_BROADCAST_HOST);
    ...
    QM_ESL2_LOG("Broadcast message sent: %s\n", msg);
...
void * consume_message(void * arg){
    ...
    if(master_group_id_ptr == NULL){
        QM_ESL2_LOG("consume_message master_group_id_ptr is NULL\n");
        free(message);
        return NULL;
    }
    master_group_id_ptr += 2;
    int master_group_id = atoi(master_group_id_ptr);
    // QM_ESL2_LOG("consume_message master_group_id: %d, local_group_id: %d\n", master_group_id, local_group_id);
    /* if(master_group_id != local_group_id && local_group_id !=0){
        QM_ESL2_LOG("consume_message master_group_id:%d != local_group_id:%d, free message\n", master_group_id, local_group_id);
        free(message);
        return NULL;
    } */
    //
    // QM_ESL2_LOG("consume_message master_group_id_ptr: %s\n", master_group_id_ptr);
    memset(temp_string, 0, sizeof(temp_string));
    char *master_ip_ptr = str_find_ip(message, temp_string);
    if (master_ip_ptr == NULL) {
        QM_ESL2_LOG("consume_message master_ip_ptr is NULL, free message\n");
        free(message);
        return NULL;
    }
    // QM_ESL2_LOG("consume_message master_ip_ptr: %s\n", master_ip_ptr);
    ...
    if(ip == NULL){
        QM_ESL2_LOG("consume_message get_local_ip is NULL, free message\n");
        free(message);
        return NULL;
    }
    ...
    if(local_group_id_1 == 0 || local_group_id_1 == master_group_id){
        ...
        if (!is_string_empty(device_sn)){
            ...
            QM_ESL2_LOG("create_send_socket_slave : %s,%s\n", master_ip, to_send_string);
            QM_ESL2_LOG("consume_message master_group_id:%d == local_group_id_1:%d \n", master_group_id, local_group_id_1);
    ...
    if(local_group_id_2 == 0 || local_group_id_2 == master_group_id){
        ...
        if (!is_string_empty(device_sn)){
            ...
            create_send_socket_slave(master_ip, to_send_string);
            QM_ESL2_LOG("create_send_socket_slave : %s,%s\n", master_ip, to_send_string);
            QM_ESL2_LOG("consume_message master_group_id:%d == local_group_id_2:%d \n", master_group_id, local_group_id_2);
...
void recevice_msg_and_send(int group_id_1,int group_id_2){
    ...
    // 绑定套接字到接收地址
    QM_ESL2_LOG("recevice_msg_and_send sin_port: %d\n",OPEN_BROADCAST_HOST);
    ...
    while(1){
        ...
        buffer[recv_len] = '\0';
        QM_ESL2_LOG("========after recvfrom msg: %s\n", buffer);
