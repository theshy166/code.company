...
{
    LV_LOG_USER("----xos_h5esl_entry-->in.");
...
static void _do_create_ui_stage2(void *p)
{
    ...
    LV_LOG_USER("----_do_create_ui_stage2-->in.");
    ...
    xos_esl_ui_init_2();
#endif
    LV_LOG_USER("----_do_create_ui_stage2-->out.");
...
static void* _first_start_ui_function(void *p)
{
    long current_time = get_milliseconds();
    printf("============= func:%s ,line:%d ==================当前时间 current_time:%ld\n",__func__, __LINE__,current_time);
    LV_LOG_USER("----_first_start_ui_function-->in.");
    usleep(100*1000);
    lv_async_call(_do_create_ui_stage2, NULL);
    LV_LOG_USER("----_first_start_ui_function-->out.");
    current_time = get_milliseconds();
    printf("============= func:%s ,line:%d ==================当前时间 current_time:%ld\n",__func__, __LINE__,current_time);
    return NULL;
}
