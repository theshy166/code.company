...
static void LV_ATTRIBUTE_FAST_MEM argb8888_image_blend(_lv_draw_sw_blend_image_dsc_t * dsc)
{
    ...
    if(dsc->blend_mode == LV_BLEND_MODE_NORMAL) {
        if(mask_buf == NULL && opa >= LV_OPA_MAX) {
            // LV_LOG_USER("blend_mode==LV_BLEND_MODE_NORMAL, dest_buf: %p", dsc->dest_buf);
            // #if defined(BUILD_RTT) && BUILD_RTT == 1
            extern int qm_effect_blend_normal_to_argb8888(_lv_draw_sw_blend_image_dsc_t * dsc);
            // int supplement = 0;
            if (LV_RESULT_OK == qm_effect_blend_normal_to_argb8888(dsc)) {
                // LV_LOG_USER("qm_effect_blend_normal_to_argb8888.");
                return;
            }
            // #endif
            if(LV_RESULT_INVALID == LV_DRAW_SW_ARGB8888_BLEND_NORMAL_TO_ARGB8888(dsc)) {
                // printf("LV_RESULT_INVALID == do_width_cpu(dsc).\n");
                for(y = 0; y < h; y++) {
                    for(x = 0; x < w; x++) {
