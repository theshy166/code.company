...
int32_t lv_obj_get_scroll_bottom(lv_obj_t * obj)
{
    ...
    if(child_res != LV_COORD_MIN) {
        int32_t bottom_bound = obj->coords.y2 - space_bottom;
        const lv_obj_t * par = obj;

        do {
            par = lv_obj_get_parent(par);
            if (par != NULL && lv_area_get_height(&par->coords) != 0 && bottom_bound > par->coords.y2) {
                bottom_bound = par->coords.y2;
            }
        } while(par != NULL);

        child_res -= bottom_bound;
    }
...
static void scroll_y_anim(void * obj, int32_t v)
{
    ...
    int32_t new_y = v + cur_y;
    int32_t adjust_y = new_y;

    if(lv_obj_has_flag(obj, LV_OBJ_FLAG_WIDGET_2))
        adjust_y = 2 * (new_y / 2);

    LV_LOG_INFO("v=%d cur_y=%d new_y=%d adjust_y=%d\n", v, cur_y, new_y, adjust_y);
    _lv_obj_scroll_by_raw(obj, 0, adjust_y);
}
