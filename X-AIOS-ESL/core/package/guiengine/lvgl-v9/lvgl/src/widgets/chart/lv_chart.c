...
void lv_chart_get_point_pos_by_id(lv_obj_t * obj, lv_chart_series_t * ser, uint32_t id, lv_point_t * p_out)
{
    ...
    if(chart->type == LV_CHART_TYPE_LINE) {
    ...
    else if(chart->type == LV_CHART_TYPE_BAR || chart->type == LV_CHART_TYPE_BAR_OVERLAP) {
        ...
        if( chart->type == LV_CHART_TYPE_BAR_OVERLAP ){
...
int32_t lv_chart_get_first_point_center_offset(lv_obj_t * obj)
{
    ...
    if(chart->type == LV_CHART_TYPE_BAR  || chart->type == LV_CHART_TYPE_BAR_OVERLAP) {
...
static void lv_chart_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_PRESSED) {
        ...
        if(_lv_ll_is_empty(&chart->series_ll) == false) {
            ...
            else if(chart->type == LV_CHART_TYPE_BAR  || chart->type == LV_CHART_TYPE_BAR_OVERLAP) draw_series_bar(obj, layer);
...
static uint32_t get_index_from_x(lv_obj_t * obj, int32_t x)
{
    ...
    if(chart->type == LV_CHART_TYPE_BAR  || chart->type == LV_CHART_TYPE_BAR_OVERLAP) return (x * chart->point_cnt) / w;
