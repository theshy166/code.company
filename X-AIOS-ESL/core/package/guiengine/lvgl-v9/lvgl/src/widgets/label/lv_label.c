...
#define LV_LABEL_SCROLL_EVEN_SHORT_WIDTH 1
...
static void lv_label_refr_text(lv_obj_t * obj)
{
    ...
    label->size_cache = size;
    label->width_cache = max_w;
    ...
    if(label->long_mode == LV_LABEL_LONG_SCROLL) {
        ...
        bool hor_anim = false;
        if(LV_LABEL_SCROLL_EVEN_SHORT_WIDTH || size.x > lv_area_get_width(&txt_coords)) {
            int32_t start = 0;
            int32_t end = 0;
