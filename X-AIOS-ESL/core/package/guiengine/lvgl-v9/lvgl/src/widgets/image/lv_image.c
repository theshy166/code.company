...
void lv_image_set_src2(lv_obj_t * obj, const void * src2)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);

    lv_obj_invalidate(obj);

    lv_image_src_t src2_type = lv_image_src_get_type(src2);
    lv_image_t * img = (lv_image_t *)obj;

    if(src2_type == LV_IMAGE_SRC_UNKNOWN) {
        LV_LOG_INFO("unknown image type");
        if(img->src2_type == LV_IMAGE_SRC_SYMBOL || img->src2_type == LV_IMAGE_SRC_FILE) {
            lv_free((void *)img->src2);
        }
        img->src2      = NULL;
        img->src2_type = LV_IMAGE_SRC_UNKNOWN;
        img->src2_area.x2 = LV_COORD_MIN;
        img->dst2_area.x2 = LV_COORD_MIN;
        return;
    }

    lv_image_header_t header;
    lv_result_t res = lv_image_decoder_get_info(src2, &header);
    if(res != LV_RESULT_OK) {
#if LV_USE_LOG
        char buf[24];
        LV_LOG_WARN("failed to get image info: %s",
                    src2_type == LV_IMAGE_SRC_FILE ? (const char *)src2 : (lv_snprintf(buf, sizeof(buf), "%p", src2), buf));
#endif /*LV_USE_LOG*/
        return;
    }

    /*Save the source*/
    if(src2_type == LV_IMAGE_SRC_VARIABLE) {
        /*If memory was allocated because of the previous `src_type` then free it*/
        if(img->src2_type == LV_IMAGE_SRC_FILE || img->src2_type == LV_IMAGE_SRC_SYMBOL) {
            lv_free((void *)img->src2);
        }
        img->src2 = src2;
        img->src2_area.x2 = LV_COORD_MIN;
    ...
    else if(src2_type == LV_IMAGE_SRC_FILE || src2_type == LV_IMAGE_SRC_SYMBOL) {
        /*If the new and the old src are the same then it was only a refresh.*/
        if(img->src2 != src2) {
            const void * old_src2 = NULL;
            /*If memory was allocated because of the previous `src_type` then save its pointer and free after allocation.
             *It's important to allocate first to be sure the new data will be on a new address.
             *Else `img_cache` wouldn't see the change in source.*/
            if(img->src2_type == LV_IMAGE_SRC_FILE || img->src2_type == LV_IMAGE_SRC_SYMBOL) {
            ...
            char * new_str = lv_strdup(src2);
            LV_ASSERT_MALLOC(new_str);
            if(new_str == NULL) return;
            img->src2 = new_str;

            if(old_src2) lv_free((void *)old_src2);
            img->src2_area.x2 = LV_COORD_MIN;
            img->dst2_area.x2 = LV_COORD_MIN;
        }
    }

    if(src2_type == LV_IMAGE_SRC_SYMBOL) {
        /*`lv_image_dsc_get_info` couldn't set the width and height of a font so set it here*/
        const lv_font_t * font = lv_obj_get_style_text_font(obj, LV_PART_MAIN);
        int32_t letter_space = lv_obj_get_style_text_letter_space(obj, LV_PART_MAIN);
        int32_t line_space = lv_obj_get_style_text_line_space(obj, LV_PART_MAIN);
        lv_point_t size;
        lv_text_get_size(&size, src2, font, letter_space, line_space, LV_COORD_MAX, LV_TEXT_FLAG_NONE);
        header.w = size.x;
        header.h = size.y;
    }

    img->src2_type = src2_type;
    img->w2        = header.w;
    img->h2        = header.h;
    img->cf2       = header.cf;
...
void lv_image_set_area(lv_obj_t * obj, const lv_area_t *src_area, const lv_area_t *dst_area)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    ...
    if (src_area)
        img->src_area = *src_area;

    if (dst_area)
        img->dst_area = *dst_area;
}

void lv_image_set_area2(lv_obj_t * obj, const lv_area_t *src2_area, const lv_area_t *dst2_area)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    ...
    if (src2_area)
        img->src2_area = *src2_area;

    if (dst2_area)
        img->dst2_area = *dst2_area;
}
...
static void lv_image_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    img->src2        = NULL;
    img->src2_type  = LV_IMAGE_SRC_UNKNOWN;
    img->cf2        = LV_COLOR_FORMAT_UNKNOWN;
    img->w2         = lv_obj_get_width(obj);
    img->h2         = lv_obj_get_height(obj);
    img->src2_area.x2 = LV_COORD_MIN;
    img->dst2_area.x2 = LV_COORD_MIN;
...
static void lv_image_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_STYLE_CHANGED) {
        ...
        if(img->src_type == LV_IMAGE_SRC_SYMBOL || img->src2_type == LV_IMAGE_SRC_SYMBOL) {
            if(img->src_type == LV_IMAGE_SRC_SYMBOL)
                lv_image_set_src(obj, img->src);

            if(img->src2_type == LV_IMAGE_SRC_SYMBOL)
                lv_image_set_src2(obj, img->src2);
        }
        else {
...
static void draw_image(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_COVER_CHECK) {
        ...
        if(img->src_type == LV_IMAGE_SRC_FILE || img->src_type == LV_IMAGE_SRC_VARIABLE) {
            ...
            if (img->src_area.x2 != LV_COORD_MIN) {
                draw_dsc.original_area.x1 = img->src_area.x1;
                draw_dsc.original_area.y1 = img->src_area.y1;
                if (img->src_area.x2 > img->src_area.x1 + img->w - 1)
                    draw_dsc.original_area.x2 = img->src_area.x1 + img->w - 1;
                else
                    draw_dsc.original_area.x2 = img->src_area.x2;

                if (img->src_area.y2 > img->src_area.y1 + img->h - 1)
                    draw_dsc.original_area.y2 = img->src_area.y1 + img->h - 1;
                else
                    draw_dsc.original_area.y2 = img->src_area.y2;

                LV_LOG_INFO("w=%d h=%d src_area=[%d,%d,%d,%d] original_area=[%d,%d,%d,%d]",
                    img->w, img->h, img->src_area.x1, img->src_area.y1, img->src_area.x2, img->src_area.y2,
                    draw_dsc.original_area.x1, draw_dsc.original_area.y1, draw_dsc.original_area.x2, draw_dsc.original_area.y2);
            }

            if (img->dst_area.x2 != LV_COORD_MIN) {
                _lv_area_intersect(&img_area, &img->dst_area, &obj->coords);
                LV_LOG_INFO("dst_area=[%d,%d,%d,%d] coords=[%d,%d,%d,%d] img_area=[%d,%d,%d,%d]",
                    img->dst_area.x1, img->dst_area.y1, img->dst_area.x2, img->dst_area.y2,
                    obj->coords.x1, obj->coords.y1, obj->coords.x2, obj->coords.y2,
                    img_area.x1, img_area.y1, img_area.x2, img_area.y2);
            }

            // printf("---disp=%p. disp_refresh_allowed=%d. disp_screen_index=%d\n", disp, disp->refresh_allowed, disp->screen_index);

            //
            if (!disp->refresh_allowed) {
                // lv_image_src_t src_type = lv_image_src_get_type(img->src);
                // if(src_type == LV_IMAGE_SRC_VARIABLE) {
                //     const lv_image_dsc_t * img_dsc = img->src;
                //     if (!(img_dsc->header.flags & LV_IMAGE_FLAGS_USER1)) {
                //         // LV_LOG_USER("---refresh not allowed, but still refresh");
                //         lv_draw_image(layer, &draw_dsc, &img_area);
                //     }
                //     else {
                //         LV_LOG_USER("---refresh not allowed, so ignore it.");
                //     }
                // }
            }
            else
        ...
        if(img->src2_type == LV_IMAGE_SRC_FILE || img->src2_type == LV_IMAGE_SRC_VARIABLE) {
            lv_draw_image_dsc_t draw_dsc;
            lv_draw_image_dsc_init(&draw_dsc);
            lv_obj_init_draw_image_dsc(obj, LV_PART_MAIN, &draw_dsc);
            ...
            draw_dsc.blend_mode = img->blend_mode;
            draw_dsc.bitmap_mask_src = img->bitmap_mask_src;
            draw_dsc.src = img->src2;

            lv_area_t img_area = {obj->coords.x1, obj->coords.y1,
                                  obj->coords.x1 + img->w2 - 1, obj->coords.y1 + img->h2 - 1
                                 };
            if(img->align < _LV_IMAGE_ALIGN_AUTO_TRANSFORM) {
                lv_area_align(&obj->coords, &img_area, img->align, img->offset.x, img->offset.y);
            }
                ...
                lv_area_move(&img_area,
                             ((layer->_clip_area.x1 - img_area.x1 - (img->w2 - 1)) / img->w2) * img->w2,
                             ((layer->_clip_area.y1 - img_area.y1 - (img->h2 - 1)) / img->h2) * img->h2);
                draw_dsc.tile = 1;
            }

            if (img->src2_area.x2 != LV_COORD_MIN) {
                draw_dsc.original_area.x1 = img->src2_area.x1;
                draw_dsc.original_area.y1 = img->src2_area.y1;
                if (img->src2_area.x2 > img->src2_area.x1 + img->w2 - 1)
                    draw_dsc.original_area.x2 = img->src2_area.x1 + img->w2 - 1;
                else
                    draw_dsc.original_area.x2 = img->src2_area.x2;

                if (img->src2_area.y2 > img->src2_area.y1 + img->h2 - 1)
                    draw_dsc.original_area.y2 = img->src2_area.y1 + img->h2 - 1;
                else
                    draw_dsc.original_area.y2 = img->src2_area.y2;

                LV_LOG_INFO("w2=%d h2=%d src2_area=[%d,%d,%d,%d] original_area=[%d,%d,%d,%d]",
                    img->w2, img->h2, img->src2_area.x1, img->src2_area.y1, img->src2_area.x2, img->src2_area.y2,
                    draw_dsc.original_area.x1, draw_dsc.original_area.y1, draw_dsc.original_area.x2, draw_dsc.original_area.y2);
            }

            if (img->dst2_area.x2 != LV_COORD_MIN) {
                _lv_area_intersect(&img_area, &img->dst2_area, &obj->coords);
                LV_LOG_INFO("dst2_area=[%d,%d,%d,%d] coords=[%d,%d,%d,%d] img_area=[%d,%d,%d,%d]",
                    img->dst2_area.x1, img->dst2_area.y1, img->dst2_area.x2, img->dst2_area.y2,
                    obj->coords.x1, obj->coords.y1, obj->coords.x2, obj->coords.y2,
                    img_area.x1, img_area.y1, img_area.x2, img_area.y2);
            }
        ...
        else if(img->src2_type == LV_IMAGE_SRC_SYMBOL) {
            lv_draw_label_dsc_t label_dsc;
            lv_draw_label_dsc_init(&label_dsc);
            lv_obj_init_draw_label_dsc(obj, LV_PART_MAIN, &label_dsc);
            label_dsc.text = img->src2;
            lv_draw_label(layer, &label_dsc, &obj->coords);
        }
    }
}
