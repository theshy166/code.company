...
#define disp_1st LV_GLOBAL_DEFAULT()->disp_1st_scr
#define disp_2nd LV_GLOBAL_DEFAULT()->disp_2nd_scr
...
lv_display_t * lv_display_create(int32_t hor_res, int32_t ver_res)
{
    ...
    disp->dpi              = LV_DPI_DEF;
    disp->color_format = LV_COLOR_FORMAT_NATIVE_WITH_ALPHA;
    ...
    lv_obj_remove_flag(disp->act_scr, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_remove_flag(disp->top_layer, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_remove_flag(disp->sys_layer, LV_OBJ_FLAG_SCROLLABLE);
...
void lv_display_set_1st_scr(lv_display_t * disp)
{
    disp_1st = disp;
}
...
void lv_display_set_2nd_scr(lv_display_t * disp)
{
    disp_2nd = disp;
}
...
void lv_display_set_refresh_allowed(lv_display_t * disp_p, bool allowed) {
    if (disp_p == NULL) return;
    disp_p->refresh_allowed = allowed;
}

void lv_display_set_screen_index(lv_display_t * disp_p, int index) {
    if (disp_p == NULL) return;
    disp_p->screen_index = index;
}
