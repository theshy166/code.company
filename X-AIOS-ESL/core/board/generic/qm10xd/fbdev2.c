...
extern unsigned long g_lvgl_phy2;
extern void * g_lvgl_virt2;
...
extern char* fbdev2_get_fbp(void);
extern void fbdev2_get_info(struct fb_var_screeninfo* pOutScreenInfo, struct fb_fix_screeninfo *pOutFixInfo);
extern int  fbdev2_pan_disp(void);
//
void fbdev2_init(void)
{
    extern void fbdev2_init_xd(void);
    fbdev2_init_xd();
    fbp = fbdev2_get_fbp();
    fbdev2_get_info(&vinfo, &finfo);
    fb_surface.data = fbp;
    ...
    lvgl_surface.phy_addr = g_lvgl_phy2;
    lvgl_surface.data = g_lvgl_virt2;
...
{
    printf("%s e\n", __func__);
    extern void fbdev2_exit_xd(void);
    fbdev2_exit_xd();
    printf("%s x\n", __func__);
}
...
void fbdev2_set_statusbar(int x, int y, int width, int height)
{
    ...
    if (ret == QUA_FALSE) {
        ...
        if (area->y1 < 160 && area->x1 == 0) {
            ...
            printf("%s CPU fallback: pixel0 src_rgba=[%02x,%02x,%02x,%02x] dst_argb1555=%04x rows=%d lvgl=%p fb=%p\n",
                   __func__, sp0[2], sp0[1], sp0[0], sp0[3], *dp0, h,
