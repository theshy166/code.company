...
void analog_write_register_bit(vou_reg_base_t *regbase, uint32_t base_addr, uint32_t offset, uint32_t value, uint32_t start, uint32_t bit)
{
	writelb(value, (uint32_t)(regbase->analog + base_addr + offset), start, bit);
}
