 ...
 */
static void dump_mem(const char *str, unsigned long bottom,
                     unsigned long top)
{
	unsigned long first;
	int i;
...
{
	int i;
	char str[sizeof("00000000 ") * 9 + 2 + 1], *k = str;
	unsigned long p = regs->ARM_pc - 16;
	unsigned long val;
...
void show_regs (struct pt_regs *regs)
{
	...
	printf ("  IRQs %s  FIQs %s  Mode %s%s\n",
		...
		thumb_mode (regs) ? " (T)" : "");
	dump_mem("stack: ",regs->ARM_sp,((regs->ARM_sp & 0xFFFFF000) + 0x1000));
	dump_instr(regs);
}
