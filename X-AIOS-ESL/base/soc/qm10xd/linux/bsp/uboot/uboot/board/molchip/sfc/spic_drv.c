...
void sfc_access_start(void)
{
	...
	do
	{
		...
		if(status == 0x3) {
			status = spi_writel(REG_SPI_ICR,0x3);//clear interrupt
			break;
		}
...
void sfc_dma_write_data(u8 *src,u32 len)
{
	...
	dma_chn_src(DMA_CHN31,(unsigned long)src);
...
void sfc_dma_read_data(u8 *dest,u32 len)
{
	...
	dma_chn_dst(DMA_CHN31,(unsigned long)dest);
	dma_trsc_len(DMA_CHN31,len);
}
...
static unsigned int sc_debug =0;
