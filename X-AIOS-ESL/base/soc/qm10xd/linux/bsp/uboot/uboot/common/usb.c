...
int usb_init(void)
{
	...
	for (i = 0; i < CONFIG_USB_MAX_CONTROLLER_COUNT; i++) {
		...
		if (start_index == dev_index) {
			...
			int j;
			for(j = 0; j < dev_index - start_index; j++){//Find The First Mass Storage Only.
				if(usb_dev[j].config.if_desc[0].desc.bInterfaceClass == USB_CLASS_MASS_STORAGE) {
				usb_started = 1;
					goto Found_MassStorage;	
				}
			}
		}
