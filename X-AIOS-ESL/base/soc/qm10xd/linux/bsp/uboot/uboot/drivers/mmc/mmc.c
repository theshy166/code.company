...
static int mmc_change_freq(struct mmc *mmc)
{
	...
	if (cardtype & EXT_CSD_CARD_TYPE_52) {
		if (cardtype & EXT_CSD_CARD_TYPE_DDR_1_8V)
			mmc->card_caps |= MMC_MODE_DDR_52MHz |MMC_MODE_EMMC_HS400 |MMC_MODE_EMMC_HS200;
...
static int mmc_startup(struct mmc *mmc)
{
	...
	if (IS_SD(mmc)) {
		...
		static unsigned ext_to_hostcaps[] = {
			[EXT_CSD_DDR_DS_BUS_WIDTH_8] =
				MMC_MODE_EMMC_HS400 | MMC_MODE_8BIT,
			[EXT_CSD_DDR_BUS_WIDTH_4] =
			...
			[EXT_CSD_BUS_WIDTH_8] =
				MMC_MODE_EMMC_HS200 |MMC_MODE_8BIT,
		};
		...
		for (idx=0; idx < ARRAY_SIZE(ext_csd_bits); idx++) {
			...
			if ((mmc->card_caps & caps) != caps)
				continue;
			mmc->tran_speed = 400000;
			mmc_set_clock(mmc, mmc->tran_speed);
			mmc->caps = caps;
			err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
					EXT_CSD_HS_TIMING, 0x1);
			if (err){
				printf("emmc start set High Speed mode fail\n");
				continue;
			}
			err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			...
			mmc->ddr_mode = (mmc->caps & MMC_MODE_EMMC_HS400 || mmc->caps & MMC_MODE_DDR_52MHz) ? 1 : 0;

			if (mmc->caps & MMC_MODE_EMMC_HS400){
				err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
						EXT_CSD_HS_TIMING, 0x13);
			}else if(mmc->caps & MMC_MODE_EMMC_HS200){
				err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
						EXT_CSD_HS_TIMING, 0x2);
				printf("try emmc run HS200 mode\n");
			}else{
				printf("try emmc run High Speed mode\n");
				err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,EXT_CSD_HS_TIMING, 0x1);
			}
			if (err){
				continue;
			}
			...
			}else if(mmc->caps & MMC_MODE_HS_52MHz || mmc->caps & MMC_MODE_DDR_52MHz){
    		...
    		if (err){
    			printf("send status error, err =%d\n", err);
    			continue;
    		}
			err = mmc_send_ext_csd(mmc, test_csd);
			if (err){
