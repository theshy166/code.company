...
#define MMC_DEV_STATUS_MASK		(~0x0206BF7F)
#define MMC_DEV_STATUS_PRG		7
...
#define   SDHCI_CTRL_HS200		0x0003
...
{
	writel(val, (void *)REG_EMMC_CTRL_CFG_BASE + reg);
}

static inline void sdhci_writew(u16 val, int reg)
{
	writew(val, (void *)REG_EMMC_CTRL_CFG_BASE + reg);
}

static inline void sdhci_writeb(u8 val, int reg)
{
	writeb(val, (void *)REG_EMMC_CTRL_CFG_BASE + reg);
}

static inline u32 sdhci_readl(int reg)
{
	return readl((void *)REG_EMMC_CTRL_CFG_BASE + reg);
}

static inline u16 sdhci_readw(int reg)
{
	return readw((void *)REG_EMMC_CTRL_CFG_BASE + reg);
}

static inline u8 sdhci_readb(int reg)
{
	return readb((void *)REG_EMMC_CTRL_CFG_BASE + reg);
}

#define SDHCI_MAX_DIV_SPEC_300	2046
#define SDHCI_CLK_BASE			240000000

#define EMMC_WAIT_TIMEOUT		10000000
#define EMMC_SDMA_SIZE_LIMIT	0x8000000
#endif
/*
...
#define BUS_HZ					24000000
#define TRANS_DATA_FREQS		24000000
#define HS_DATA_FREQS			52000000
#define HS400_DATA_FREQS		200000000
...
#define		EMMC_BASE_SEL_24M		0
#define		EMMC_BASE_SEL_200M		1
#define		EMMC_BASE_SEL_297M		2
#define		EMMC_BASE_SEL_400M		3
...
#define		EMMC_PHY_DELAY_LINE_HS200		0x202018
#define		EMM_PHY_DELAY_LINE_HS400		0x080808
...
int spl_emmc_load_image(struct spl_image_info *spl_image);
int sched_emmc_load_image(struct boot_image_info * info);
int sched_emmc_fast_load_image(boot_type_t type);
...
extern const unsigned freqs[4];
extern u8 ext_csd[512];
extern unsigned int clk_emmc_2x[4];
