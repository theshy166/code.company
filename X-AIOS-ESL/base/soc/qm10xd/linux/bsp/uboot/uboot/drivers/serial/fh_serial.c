 ...
 *
 *********************************/
#define	DBGLINE() 		printk(KERN_DEBUG"file: %s\tfunc:%s\tline:%d\n",__FILE__, __FUNCTION__,__LINE__)
...
//static int fh_uart_rx_fifocnt(struct uart_port *port);
...
fh_uart_rx_chars(int irq, void *dev_id)
{
	...
	while (max_count-- > 0) {
		...
		ret_status = (u8)Uart_Read_Control_Status(&myown_port->fh_info);
		if(!(ret_status & UART_STATUS_RFNE))//0 means empty.	 1 means not empty
			break;
