...
data=$(dd if="u-boot-spl-header.img" bs=1 skip=8 count=4 2>/dev/null | xxd -p)
...
# secure addr
secure_addr=$(dd if="u-boot-spl-header.img" bs=1 skip=4 count=4 2>/dev/null | xxd -p)
...
dd if="u-boot-spl-header.img" of="spl.bin" bs=1 skip=0 count=$count_decimal 2>/dev/null

# 获取新文件的大小，如果不是 16 字节对齐，则填充 0 使其对齐
file_size=$(stat -c%s "spl.bin")
if (( file_size % 16 != 0 )); then
    # 计算需要填充的字节数
    padding=$(( 16 - file_size % 16 ))
    dd if=/dev/zero bs=1 count=$padding >> spl.bin 2>/dev/null
fi

# 打印 spl.bin 的最终大小
final_size=$(stat -c%s "spl.bin")
echo "Final spl.bin size: $final_size bytes (aligned to 16 bytes)"
...
# 将反转后的对齐值写入 spl.bin 文件的第8字节位置
echo "$reversed_aligned_hex" | xxd -r -p | dd of="spl.bin" bs=1 seek=8 conv=notrunc 2>/dev/null
...
# 将反转后的对齐值写入 spl.bin 文件的第16字节位置
echo "$reversed_aligned_hex" | xxd -r -p | dd of="spl.bin" bs=1 seek=16 conv=notrunc 2>/dev/null
...
# 将反转后的对齐值写入 spl.bin 文件的第12字节位置
echo "$reversed_aligned_hex" | xxd -r -p | dd of="spl.bin" bs=1 seek=12 conv=notrunc 2>/dev/null
# 输出最终 spl.bin 文件的内容
echo "Updated spl.bin with aligned dtb offset at position 8."
