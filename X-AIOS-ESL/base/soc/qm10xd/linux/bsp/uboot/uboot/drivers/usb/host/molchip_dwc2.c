...
#include "dwc2.h"
#include "molchip_dwc2.h"

DECLARE_GLOBAL_DATA_PTR;
#define USB_CEN_PERI_CLK_CTRL	0x0c0000ac
#define USB2_APB_CFG_REG  	0x0B600000

#define CONFIG_USB_DWC2_REG_ADDR 0x0ca00000
...
struct dwc2_priv {
	...
	uint8_t *aligned1_buffer;
	uint8_t *status1_buffer;
#endif
...
DEFINE_ALIGN_BUFFER(uint8_t, status_buffer_addr, DWC2_STATUS_BUF_SIZE,
		ARCH_DMA_MINALIGN);
DEFINE_ALIGN_BUFFER(uint8_t, aligned1_buffer_addr, DWC2_DATA_BUF_SIZE,
		ARCH_DMA_MINALIGN);
DEFINE_ALIGN_BUFFER(uint8_t, status1_buffer_addr, DWC2_STATUS_BUF_SIZE,
		ARCH_DMA_MINALIGN);
...
static int dwc2_init_common(struct udevice *dev, struct dwc2_priv *priv)
{
	...
	reg = readl(USB_CEN_PERI_CLK_CTRL);
	reg |= (BIT(0) | BIT(1));
	writel(reg, USB_CEN_PERI_CLK_CTRL);
...
//	__raw_writel(reg, USB2_APB_VBUS_REG);
	...
	reg = readl(USB2_APB_CFG_REG);
	reg &= ~(BIT(3) | BIT(4));
	writel(reg, USB2_APB_CFG_REG);
	...
	reg &= ~(1 << 6);
	writel(reg, USB2_APB_CFG_REG);
	//int index = ((unsigned int)regs == CONFIG_USB_DWC2_REG_ADDR) ? 0:1;
	//molchip_usb2_enable_phy(index);
	...
	if ((snpsid & DWC2_SNPSID_DEVID_MASK) != DWC2_SNPSID_DEVID_VER_2xx &&
		...
		printf("SNPSID invalid (not Molchip DWC2 OTG device): %08x\n", snpsid);
		return -ENODEV;
	}
...
static const struct udevice_id dwc2_usb_ids[] = {
	...
	{ .compatible = "molchip,dwc2" },
	{ }
};

U_BOOT_DRIVER(usb_molchip_dwc2) = {
	.name	= "dwc2_molchip_usb",
	.id	= UCLASS_USB,
