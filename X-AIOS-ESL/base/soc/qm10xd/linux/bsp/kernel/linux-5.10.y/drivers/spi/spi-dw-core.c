			  ...
			  struct dw_spi_cfg *cfg)
{
	...
	clk_div = (DIV_ROUND_UP(dws->max_freq, cfg->freq)) & 0xfffe;
...
static int dw_spi_setup(struct spi_device *spi)
{
	...
	device_property_read_u32(&spi->dev, "rx-sample-delay", &chip->rx_sample_dly);
...
void spi_hw_disable(struct dw_spi *dws)
{
...
static int dw_spi_eco_set(struct dw_spi *dws)
{
	void *addr = NULL;
	int val;
	addr = ioremap(REG_CPU_SYS_AHB_BASE,0x1000);
	val = __raw_readl(addr + CPU_APB_ECO_REG0);
	if (!dws->slave_mode) {
		if (dws->id == SPI_DEV0)
			val |= SPI0_ECO;
	...
	__raw_writel(val,addr + CPU_APB_ECO_REG0);
	iounmap(addr);
	return 0;
}
