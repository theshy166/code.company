...
static int  gesture_last=0,gesture_value=0;
...
static int gesture_min_x,gesture_min_y;
static unsigned int size_ratio;
typedef struct
{
	int dis;
	int count;
...
static int gesture_multi = 0;
static unsigned int multi_set = 0;
static unsigned int gesture_flag = 0;
...
static unsigned int (*ReadIICInt) (unsigned int *data,unsigned int addr,unsigned int len); 
...
={
	...
	0x0004ccb7,0x0e03f2e0,0x3621fffd,0x5b4aecf9,0x776acbdc,0x797ba1b6,0x566b8a90,0x2c41928b,}},
	{0x10,'6',{
	0x18009fa5,0x4930969a,0x79619193,0xaa929190,0xdbc39893,0xfdf0baa3,0xeafae5d1,0xbfd6fbf4,
	0x8ea6ffff,0x6477eaf9,0x5356bcd5,0x5f588ea5,0x756a6176,0x94843a4c,0xb9a61829,0xe5cd000a,}},
	{0x10,'7',{
	...
	{0x10,'8',{
	0x76547489,0xbb984e62,0xe9d71033,0x9dc30005,0x50760100,0x0b291e08,0x2a095943,0x734e7868,
	0xbc969588,0xfae0bfa4,0xd2f5f5e3,0x85abfefb,0x385ef6fe,0x0216c4e6,0x2f0e8ea5,0x77716e7b,}},
	{0x10,'9',{
	...
	{0x10,'A',{
	0x5b790106,0x293d240c,0x0e195e41,0x02069d7e,0x0400dcbd,0x3315f8f7,0x5d4ac8e2,0x7b6c90ad,
	0x8e855372,0x9b951434,0x9f9f3717,0x9f9f7657,0xa8a3b596,0xb9aff3d5,0xe2d1d5ef,0xfff29ab8}},
	{0x10,'B',{
	...
	{0x10,'B',{
	0x0c0c1f00,0x12105f3f,0x0c0f9f7f,0x0208dfbf,0x0200d1f1,0x1b0b95b2,0x4c306b7d,0x8c6c6163,
	0xc9ab7062,0xf4e0a287,0xf0ffddc1,0xb5d5faef,0x7595fcff,0x3655ecf6,0x091ac0dc,0x0f0780a0,}},
	{0x10,'B',{
	0xbfd50d02,0x94a93420,0x697e5b48,0x3f53826f,0x1127a693,0x1802a2b5,0x4b328593,0x82667279,
	0xbb9f6e6d,0xefd78677,0xf6fdbba1,0xcae3e3d2,0x94b0f8f1,0x5b77fffd,0x253eebfa,0x1114b5d2,}},
	{0x10,'C',{
	...
	0xf2fa462e,0xdfea745d,0xc4d29e89,0xa4b5c5b3,0x8093e9d9,0x526bfef7,0x2139f2fc,0x000fcae0,}},
	{0x10,'G',{
	...
	{0x10,'M',{
	0x0e00421a,0x1414936a,0x1416e3bb,0x1813c0e8,0x201b6f97,0x2f261e47,0x664d2c09,0x7a737c54,
	0x7c7bcda5,0x7e7accf5,0x8b847ba4,0x9c932b53,0xd5b30d09,0xece25d35,0xfaf5ae85,0xfffdffd6,}},
	{0x10,'M',{
	...
	0x9f952b41,0xbdab0616,0xe6d51100,0xf7f13f27,0xfcfa6f57,0xfffd9f87,0xffffcfb7,0xffffffe7,}},
	{0x10,'M'+0x100,{
	0xf1ffe0ff,0xdce8a2c0,0xc8d16282,0xbbc12343,0xaab22a0b,0x9fa46a4a,0x9298a989,0x808ae8c9,
	0x6f74bfde,0x61697f9f,0x555b4060,0x464f0120,0x363d3f1f,0x282f7e5e,0x151fbc9e,0x000afcdc,}},
	{0x10,'N',{
	...
	{0x0c,'O',{
	0x040e8a71,0x0b03bca4,0x2c19e7d4,0x5d43fef5,0x9077f9fe,0xbea8deee,0xdfcfb6cb,0xf7ed859f,
	0xfefd516b,0xe6f62237,0xb7d00812,0x839d0002,0x4f690701,0x20371f11,0x040e4e34,0x00028268,}},
	{0x0c,'O',{
	...
	{0x10,'Q',{
	0xc9e4020f,0x8dab0000,0x516f0802,0x18341f11,0x02065537,0x35186f68,0x71536b6f,0xaa8e5963,	
	0xdec53a4b,0xfef40924,0xe1ef3e24,0xc0d07157,0xabb4ac8e,0x9da4e8ca,0xc5aaeffd,0xfde0d6e2,}},
	{0x10,'S',{
	...
	{0x10,'V',{
	0x07008274,0x170f9d90,0x271fb6aa,0x382fd0c2,0x463febdd,0x534df7fa,0x635bdbe9,0x716ac1ce,
	0x827aa7b4,0x928a8e9a,0xa39a7682,0xb6ac5e6a,0xc8bf4753,0xdad1303b,0xede41924,0xfff4000d,}},
	{0x10,'V',{
	0x07031300,0x09083926,0x09095f4c,0x08088572,0x0808ab98,0x0608d2be,0x0104f8e5,0x1608e9f7,
	0x3325cfdc,0x4f41b5c2,0x6d5e9ba8,0x897b818e,0xa6986875,0xc4b54f5b,0xe1d23643,0xffef1b29,}},	
	{0x18,'V',{
	...
	0x7883e7f6,0x666fc7d6,0x595fa4b5,0x49518495,0x383f6474,0x27304254,0x161e2132,0x010d0411,}},
	{0x10,'W',{
	...
	{0x10,'W',{
	0x0500917c,0x160dbba6,0x271fe3d0,0x4032f2f9,0x544bcbdf,0x615aa0b5,0x6d687d8a,0x7c74a693,
	0x8b85d1bc,0x9d91f7e7,0xb3a9d1e4,0xc4bca8bc,0xd2cb7f93,0xe2da566a,0xf2eb2c41,0xfff80016,}},
	{0x10,'W',{
	0x00002d0b,0x0001714f,0x0902b593,0x2816f3d4,0x5844d4f1,0x796997b6,0x8a845375,0x858a0f31,
	0x7f804a28,0x7e7e8e6c,0x8a82d2b0,0xb596fcf4,0xe4d0cae7,0xfcf489ab,0xfffe4567,0xf9fd0023,}},	
	{0x18,'W',{
	...
	{0x10,'W',{
	0x0c00240b,0x2015563d,0x332b8970,0x4b416b84,0x5b533851,0x6a63061f,0x78722d14,0x817c5f46,
	0x8b869279,0x9791c4ab,0xa39ef7de,0xb6aed5ee,0xc6bea2bb,0xd8cf7089,0xede33d56,0xfff60a24,}},
	{0x10,'W',{
	0x0d003219,0x1e15654b,0x2c24987e,0x3732cbb1,0x453dfee4,0x564dcde7,0x675d9bb4,0x7b706882,
	0x9084354f,0xa39b021c,0xada83117,0xbab2644a,0xc8c0977d,0xded47791,0xf1e7445e,0xfff7112b,}},
	{0x10,'W',{
	...
	{0x10,'Y',{
	0x0b012f4b,0x42240f1a,0x494c4c2d,0x6a4d746b,0xa0865365,0xd2bb2a40,0xefe40f0f,0xfbf54e2f,
	0xfcff8d6e,0xe6f3caad,0xb8d1f5e2,0x7998fdff,0x435bdef2,0x503aa9c0,0x8b6c9099,0xcaaa8088,}},
	{0x10,'Z',{
	...
	{0x10,'<',{
	0xf0ff0b00,0xd1e11c14,0xb0c02c24,0x90a13c33,0x71804c44,0x51605c54,0x2f406d64,0x0d1e7c75,
	0x16058d86,0x38279b94,0x5747a9a2,0x7768b9b1,0x9686c9c0,0xb5a6dad1,0xd5c6ece3,0xf5e4fff6,}},
	{0x10,'>',{
	...
	0xf0ff707a,0xcfdf6268,0xadbe525a,0x8d9e434a,0x6d7d353c,0x4c5c262e,0x2c3c151e,0x0c1c000b,}},
...
static int GestureJoint(void)
{
	...
	unsigned int addr = 0;
	unsigned int i = 0;
	unsigned int fill_able = FALSE;
	pr_debug("GestureJoint1110\n");
	if((point_num &0x8000) == 0){
		return FALSE;
	}
	pr_debug("GestureJoint1112\n");
	if((point_num &0xff000000) != 0x5b000000){
		return FALSE;
	}
	pr_debug("GestureJoint1113\n");
	if(ReadIICInt == NULL)
		return FALSE;
	pr_debug("GestureJoint1114\n");
	...
	for(i=0;i<len+5;i++)
	{
		if(i<len)
		{
			...
			if( buf[i] == 0 || (buf[i] & 0xe0009000) != 0)
			{
				GestureInit();
				break;
			}
			if(point_now[0].other.fill)
			{
				fill_able = FLAG_FILL;
			}
			else
...
static int GestureMain(unsigned int data_coor[],unsigned int num)
{
	unsigned int data;
	gesture_deal = FALSE;
	if(gesture_dis_min == 0 || data_coor == NULL)
		return FALSE;
	...
	else if((num == 2 && data_coor[0] != 0 && data_coor[1] != 0)
		||  (num == 1 && data_coor[0] == 0 && data_coor[1] != 0))
	{
		if(data_coor[0] != 0 && data_coor[1] != 0)
		{
			data  = data_coor[0] & 0xffff0fff;
			data += data_coor[1] & 0xffff0fff;
			data = (data/2) & 0xffff0fff;
		}
		else
		{
			data  = data_coor[1] & 0xffff0fff;
		...
		data |= 0x00002000;
		GesturePush((GESTURE_POINT_TYPE*) (&data));
		return FALSE;
	}
	else// if(num > 1)
	{
...
static int GesturePush(GESTURE_POINT_TYPE* data)
{
	unsigned int data_i = data->all & 0xffff2fff;
	if(gesture_num >= GESTURE_BUF_SIZE)
		return FALSE;
	if(gesture_flag & 0x2)//x
		data_i = ((drv_num_nokey*64 - (data_i>>16)) <<16) + (data_i & 0xffff) + (data_i & 0x00002000);
#if (GSL_VERSION & 0xF0000000) == 0xF0000000
 	data_i = (data_i & 0xffff0000) + ((sen_num_nokey*64 - (data_i & 0xffff)) & 0xffff) + (data_i & 0x00002000);
#else
	if(gesture_flag & 0x4)//y
		data_i = (data_i & 0xffff0000) + ((sen_num_nokey*64 - (data_i & 0xffff)) & 0xffff) + (data_i & 0x00002000);
#endif
	if(gesture_flag & 0x8)//xy
		data_i = ((data_i & 0xfff)<<16) + ((data_i >> 16) & 0xfff) + (data_i & 0x00002000);
	if(gesture_num == 0)
	{
		gesture_buf[gesture_num ++].all = data_i;
		return TRUE;
	}
	if(GestureDistance((GESTURE_POINT_TYPE*) (&data_i),&gesture_buf[gesture_num-1],TRUE) <= gesture_dis_min)
		return FALSE;
	gesture_buf[gesture_num ++].all = data_i;
	return TRUE;
}
...
// 		return FALSE;
	...
	for(i=0;i<gesture_num;i++)
	{
		...
		if(gesture_buf[i].all & (0x2<<12))
			n++;
	}
	if(n > gesture_num/4 || n>4)
		return FALSE;
	if(x_max <= x_min+gesture_min_x || y_max <= y_min+gesture_min_y)
		return FALSE;
	if(size_ratio!=0)
		if((x_max-x_min)*100/(y_max-y_min)<size_ratio || (y_max-y_min)*100/(x_max-x_min)<size_ratio)
			return FALSE;
	for(i=0;i<gesture_num;i++)
	{
...
static void ChangeXY(void)
{
	...
	for(i=0;i<gesture_num && i<GESTURE_BUF_SIZE;i++)
		gesture_buf[i].all = ((gesture_buf[i].all & 0xfff) << 16) + ((gesture_buf[i].all>>16) & 0xffff) + (gesture_buf[i].all&0x00002000);
}
...
static void GestureSet(unsigned int conf[])
{
	...
	if(conf[9])
	{
		gesture_min_x = conf[9] & 0xffff;
		gesture_min_y = conf[9] >> 16;
	}
	else
	{
		gesture_min_x = 2*64;
...
static int GestureDeal(void)
{
	...
	while(1)
	{
		...
		if((gesture_deal & GESTURE_DEAL) == 0)
			return FALSE;
		gesture_deal &= ~GESTURE_DEAL;
		if((gesture_flag & 0x100) == 0)
		{
			gesture_last = GestureLRUD();
			if(gesture_last)
				break;
		}
		if(gesture_threshold[0]==1 && gesture_threshold[1]==1)
			break;
		...
		for(j=0;j<4;j++)
		{
			...
			{
				gesture_val[0] =  GestureModel(model_default,sizeof(model_default)/sizeof(model_default[0]),gesture_threshold[0],&gesture_out[0]);
				gesture_val[1] =  GestureModel(model_extern,model_extern_len,gesture_threshold[1],&gesture_out[1]);
				for(i=0;i<2;i++)
				{
...
void gsl_GestureExternInt(unsigned int* model,int len)
{
	model_extern = (GESTURE_MODEL_TYPE *)model;
	model_extern_len = len;
}
EXPORT_SYMBOL(gsl_GestureExternInt);
...
//	int x1=0,y1=0,x2=0,y2=0;
	...
	int x_refe,y_refe;
	int min_scale=5;
	int start = gesture_num/8;//gesture_num×îÐ¡Öµ4
	int end = gesture_num-start*2-1;
	int ret = 0;
	if(gesture_deal & GESTURE_XY)
	{
		gesture_deal &= ~GESTURE_XY;
	...
	middle_x = (gesture_buf[start].x + gesture_buf[end].x)/2;
	middle_y = (gesture_buf[start].y + gesture_buf[end].y)/2;
	x_refe = (int)sen_num_nokey*64/(x_scale>100 ? x_scale%100 : x_scale);
//	x_refe = (int)(gesture_screen_y ? gesture_screen_y:10*64)/(x_scale>100 ? x_scale%100 : x_scale);
	y_refe = (int)drv_num_nokey*64/(y_scale>100 ? y_scale%100 : y_scale);
//	y_refe = (int)(gesture_screen_x ? gesture_screen_x:16*64)/(y_scale>100 ? y_scale%100 : y_scale);
	x_min = gesture_buf[start].y;
	x_max = gesture_buf[start].y;
	y_min = gesture_buf[start].x;
	y_max = gesture_buf[start].x;
	for(i=start+1;i<=end;i++)
	{
		if(Abs(gesture_buf[i].x-middle_x) < x_refe)
			flag3|=0x1;
		else
			flag3|=0x2;
		if(Abs(gesture_buf[i].y-middle_y) < y_refe)
			flag3|=(0x1<<4);
		else
			flag3|=(0x2<<4);
		...
		if((int)gesture_buf[i].y-(int)gesture_buf[i-1].y>min_scale)
			...
			flag3|=(0x2<<12);
		if((int)gesture_buf[i].y < x_min)
			x_min = gesture_buf[i].y;
		if((int)gesture_buf[i].y > x_max)
			x_max = gesture_buf[i].y;
		if((int)gesture_buf[i].x < y_min)
			y_min = gesture_buf[i].x;
		if((int)gesture_buf[i].x > y_max)
			y_max = gesture_buf[i].x;
	}
//	if(1)
	{
		if((flag3==0x2031||flag3==0x2131||flag3==0x2231||flag3==0x2331))//&&(y2-y1>screen_x_max/3))
		{
			if(y_scale/100==0 || y_max-y_min<y_scale/100*64)
				ret = 0xa1fc;//up(a1,fc)
		}
		...
		{
			if(x_scale/100==0 || x_max-x_min<x_scale/100*64)
				ret = 0xa1fb;//left
		}
	...
	if(ret > 0)
	{
		...
		if(flag3 >= gesture_num/2 && flag3 >= 4)
			ret -= 4;
	}
	return ret;
}
...
#define	POINT_MAX		10
	...
	{
		s = FALSE;
		if(multi_x[5-1].count >= 0 && multi_x[4-1].count >= 0 && multi_x[5-1].count + multi_x[4-1].count >= msc)
			s |= 0xbacf;
		if(multi_o[5-1].count >= 0 && multi_o[4-1].count >= 0 && multi_o[5-1].count + multi_o[4-1].count >= msc)
			s |= 0xb7d6;
