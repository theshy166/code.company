...
static int gsl_ts_read(struct i2c_client *client, u8 addr, u8 *pdata, unsigned int datalen)
{
	...
	if (datalen > 126)
	{
		dev_info(&client->dev, "too big datalen = %d!\n", datalen);
		return -1;
	}
	...
	if (ret < 0)
	{
		dev_info(&client->dev, "set data address fail!\n");
		return ret;
	}
...
static void gsl_load_fw(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "load fw start\n");
	...
	dev_info(&client->dev, "load fw end\n");
...
static int test_i2c(struct i2c_client *client)
{
	...
	write_buf[2] = 0x56;
	write_buf[3] = 0x00;
	ret = gsl_ts_read( client, 0xf0, &read_buf[0], sizeof(read_buf) );
	if (ret < 0)
		rc --;
	else
		dev_info(&client->dev, "read 0x%02x%02x%02x%02x from reg 0xf0\n", read_buf[0],read_buf[1],read_buf[2],read_buf[3]);

	msleep(2);
	ret = gsl_ts_write(client, 0xf0, &write_buf[0], sizeof(write_buf));
	if(ret >= 0)
		dev_info(&client->dev, "write 0x%02x%02x%02x%02x to reg 0xf0 \n", write_buf[0],write_buf[1],write_buf[2],write_buf[3]);
	...
	if(ret < 0)
		...
		dev_info(&client->dev, "read 0x%02x%02x%02x%02x from reg 0xf0 after writing\n",
			read_buf[0],read_buf[1],read_buf[2],read_buf[3]);
...
static void gsl_io_control(struct i2c_client *client)
{
	...
	for(i=0;i<5;i++){
			...
			buf[2] = 0;
			buf[3] = 0;
			gsl_ts_write(client,0x84,buf,4);
	}
	msleep(50);
}
#endif
...
static void init_chip(struct i2c_client *client)
{
	...
	if(rc < 0)
	{
		dev_info(&client->dev, "------gslX680 test_i2c error------\n");
		return;
	}
	clr_reg(client);
	reset_chip(client);
...
static void check_mem_data(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "xmwuwh check 0xb0 d0=0x%x,d1=0x%x,d2=0x%x,d3=0x%x\n",read_buf[0],read_buf[1],read_buf[2],read_buf[3]);
...
static void gslX680_ts_worker(struct work_struct *work)
{
	...
	if (rc < 0)
	{
		dev_info(&ts->client->dev, "read failed\n");
...
static void gsl_monitor_worker(struct work_struct *work)
{
	...
	if(b0_counter > 1)
	{
		dev_info(&gsl_client->dev, "======read 0xb0: %x %x %x %x ======\n",read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
	...
	{
		dev_info(&gsl_client->dev, "======int_1st: %x %x %x %x , int_2nd: %x %x %x %x ======\n",int_1st[3], int_1st[2], int_1st[1], int_1st[0], int_2nd[3], int_2nd[2],int_2nd[1],int_2nd[0]);
		init_chip(gsl_client);
	}
	...
	if(dac_counter > 1)
	{
		dev_info(&gsl_client->dev, "read DAC1_0: %x %x %x %x\n",read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
		init_chip(gsl_client);
		dac_counter = 0;
	}
...
queue_monitor_work:
	queue_delayed_work(gsl_monitor_workqueue, &gsl_monitor_work, msecs_to_jiffies(2000));
}
#endif
...
static int gsl_ts_suspend(struct device *dev)
{
	...
	int i;
#endif
  	dev_info(&ts->client->dev, "I'am in gsl_ts_suspend() start\n");

#ifdef GSL_MONITOR
	dev_info(&ts->client->dev, "gsl_ts_suspend () : cancel gsl_monitor_work\n");
...
static int gsl_ts_resume(struct device *dev)
{
	...
	int i;
#endif
  	dev_info(&ts->client->dev, "I'am in gsl_ts_resume() start\n");
...
#endif
#ifdef GSL_MONITOR
	dev_info(&ts->client->dev, "gsl_ts_resume () : queue gsl_monitor_work\n");
	queue_delayed_work(gsl_monitor_workqueue, &gsl_monitor_work, 300);
#endif
	dev_info(&ts->client->dev, "gsl_ts_resume () end\n");
	enable_irq(ts->irq);
	return 0;
}
...
static int gsl_ts_early_suspend(struct tp_device *tp_d)
{
	...
	dev_info(&ts->client->dev, "[GSLX680] Enter %s\n", __func__);
...
#ifdef GSL_DELAYED_INIT
// Add this new worker function to handle deferred initialization
static void gslX680_init_worker(struct work_struct *work)
{
	struct gsl_ts *ts = container_of(work, struct gsl_ts, init_work);

	dev_info(&ts->client->dev, "starting deferred ts init\n");

	// Perform the time-consuming initialization here
	init_chip(ts->client);
	check_mem_data(ts->client);

	dev_info(&ts->client->dev, "deferred ts init completed\n");
}
#endif
			...
			const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "Enter %s, addr=0x%x.\n", __func__, client->addr);
/*
	while(timer > 0)
	{
  ...
  */
	...
	if (!ts)
		return -ENOMEM;
	dev_info(&client->dev, "==kzalloc success=\n");
	...
	//ts->screen_max_x = 1920;
	//ts->screen_max_y = 1080;

	dev_info(&client->dev, "screen_max_x =[%d] \n",ts->screen_max_x);
	dev_info(&client->dev, "screen_max_y =[%d] \n",ts->screen_max_y);
	...
	if (gpio_is_valid(ts->wake_pin)) {
		...
		if (rc != 0) {
			dev_err(&client->dev, "wake pin error\n");
			return -EIO;
		}
	...
	if (gpio_is_valid(ts->irq_pin)) {
		...
		rc = gpio_direction_input(ts->irq_pin);
		if (rc)
		{
	...
	if (rc < 0) {
		dev_err(&client->dev, "ts init failed\n");
...
//	msleep(20);
...
#ifndef GSL_DELAYED_INIT
	init_chip(ts->client);
	check_mem_data(ts->client);
#endif

	ts->irq=gpio_to_irq(ts->irq_pin);		//If not defined in client
	dev_info(&client->dev, "xmwuwh tp irq gpio=%d, irq=%d\n",ts->irq_pin,ts->irq);
	if (ts->irq)
	{
		//rc = request_irq(ts->irq, gsl_ts_irq, IRQ_TYPE_EDGE_FALLING, client->name, ts);
		...
		dev_info(&client->dev, "xmwuwh tp 11 irq rc=%d\n",rc);
		if (rc != 0) {
	...
	{
		dev_info(&client->dev, "gsl x680 irq req fail\n");
		goto error_req_irq_fail;
	}
...
#ifdef GSL_MONITOR
	dev_info(&client->dev, "queue gsl_monitor_workqueue\n");

	INIT_DELAYED_WORK(&gsl_monitor_work, gsl_monitor_worker);
	gsl_monitor_workqueue = create_singlethread_workqueue("gsl_monitor_workqueue");
	if (!gsl_monitor_workqueue) {
		dev_err(&client->dev, "could not create monitor workqueue\n");
		goto error_create_monitor_wq;
	}
	queue_delayed_work(gsl_monitor_workqueue, &gsl_monitor_work, msecs_to_jiffies(5000));
#endif

#ifdef GSL_DELAYED_INIT
	// Create a separate workqueue for initialization
	ts->init_wq = create_singlethread_workqueue("gsl_init_wq");
	if (!ts->init_wq) {
		dev_err(&client->dev, "could not create init workqueue\n");
		goto error_create_init_wq;
	}
	// Initialize the work structure
	INIT_WORK(&ts->init_work, gslX680_init_worker);
	// Schedule the initialization work with a delay to not impact boot time
	queue_work(ts->wq, &ts->init_work);
#endif
...
*/
	...
	dev_info(&client->dev, "[GSLX680] End %s\n", __func__);

	return 0;

#ifdef GSL_DELAYED_INIT
error_create_init_wq:
#endif
#ifdef GSL_MONITOR
	if (gsl_monitor_workqueue)
		destroy_workqueue(gsl_monitor_workqueue);
error_create_monitor_wq:
#endif
//exit_set_irq_mode:
error_req_irq_fail:
	free_irq(ts->irq, ts);
...
static int gsl_ts_remove(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "==gsl_ts_remove=\n");
