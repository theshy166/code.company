...
/*
 * A new device capability type and descriptor must be defined for the USB BOS
 * descriptor to return platform-specific device capabilities.
 * Defines a device capability specific to
 * a particular platform/operating system
 */
#define	USB_MSOS_20_PLATFORM_TYPE	5

struct usb_platform_set_information_descriptor {
	__u32 dwWindowsVersion; /* Windows version */
/* The length, in bytes of the MS OS 2.0 descriptor set. */
	__u16 wMSOSDescriptorSetTotalLength;
/* Vendor defined code to use to retrieve this version of
 * the MS OS 2.0 descriptor and also to set alternate
 * enumeration behavior on the device.
 */
	__u8 bMS_VendorCode;
/* A non-zero value to send to the device to indicate that the device
 * may return non-default USB descriptors for enumeration.  If the
 * device does not support alternate enumeration, this value shall be 0.
 */
	__u8 bAltEnumCode;
} __attribute__((packed));


#define USB_PLATFORM_DEVICE_CAPABILITY_DESCRIPTOR(n) \
	usb_platform_device_capability_descriptor_##n
/* PlatformCapbilityUUID: A 128-bit number that
 * uniquely identifies a platform-specific
 * capability of the device.Refer to IETF RFC 4122 for details
 * on generation of a UUID. See Microsoft OS 2.0 descriptor
 * platform capability descriptor.
 *
 * D8DD60DF-4589-4CC7-9CD2-659D9E648A9F Indicates the device
 * supports the Microsoft OS 2.0 descriptor
 */
/* CapabilityData: A variable-length field containing data associated with the
 * platform specific capability.This field may be zero bytes in
 *  length. See Microsoft OS 2.0 descriptor capability data.
 */
#define DECLARE_USB_PLATFORM_DEVICE_CAPABILITY_DESCRIPTOR(n) \
struct USB_PLATFORM_DEVICE_CAPABILITY_DESCRIPTOR(n) {\
	__u8  bLength;\
	__u8  bDescriptorType;\
	__u8  bDevCapabilityType;\
	__u8  bReserved;\
	__u8  PlatformCapbilityUUID[16];\
	struct usb_platform_set_information_descriptor CapabilityData[n];\
} __attribute__((packed))


#define USB_DT_PLATFORM_DEVICE_CAPABILITY_SIZE(n) (20 + (8 * n))
/*
