...
void ata_eh_hbareset_port(struct ata_link *link)
{
	...
	if (ap->ops->hba_reset)
		ap->ops->hba_reset(link);

	DPRINTK("ata hba reset\n");
}
		 ...
		 ata_reset_fn_t hardreset, ata_postreset_fn_t postreset)
{
 ...
 retry:
	...
	if (reset) {
		...
		if (reset == hardreset &&
	     ...
	     "follow-up softreset required but no softreset available\n");
			...
			rc = ata_do_reset(link, reset, classes, deadline, true);
			if (rc) {
				if(ata_is_ahci_active(link)){
					if (try < max_tries)
						ata_eh_hbareset_port(link);
				}
				failed_link = link;
				goto fail;
			}
		}
		if (ata_is_host_link(link) && !(link->flags & ATA_LFLAG_NO_SRST)){
			if(rc == -EAGAIN){
				failed_link = link;
				goto fail;
			}
	...
	if(ata_is_host_link(link) && *classes && ata_phys_link_offline(link)){
		failed_link = link;
		goto fail;
	}
