...
static int mc_perfmon_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	unsigned int i, reg_val, channel_num;
	int ret;
	...
	ret = clk_mini_enable(CKG_PERF_MON0_EN);
	if (ret)
		goto exit;

	ret = clk_mini_enable(CKG_PERF_MON1_EN);
	if (ret)
		goto error_clk0;

	ret = clk_mini_enable(CKG_PERF_MON2_EN);
	if (ret)
		goto error_clk1;
...
error_clk2:
	clk_mini_disable(CKG_PERF_MON2_EN);
error_clk1:
	clk_mini_disable(CKG_PERF_MON1_EN);
error_clk0:
	clk_mini_disable(CKG_PERF_MON0_EN);
exit:
	return ret;
}
                        ...
                        size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(buf+(6 << 5)+0x20,&tv,sizeof(tv))) {
			return -EFAULT;
	}
	return count;
