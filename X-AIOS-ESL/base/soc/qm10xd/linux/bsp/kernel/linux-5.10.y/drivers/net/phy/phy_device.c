			  ...
			  char *buf)
{
	...
	page = 0;
	reg = 0x1f;
	phy_write(phydev, reg, page);
	for (reg = 0; reg < 0x15; reg++)
		pr_info("page 0x%x reg 0x%02x val 0x%04x\n", page, reg, phy_read(phydev, reg));
			   ...
			   const char *buf, size_t count)
{
	...
	if (data & 0xf000000000000000) {
		page = (data & 0xfff000000) >> 24;
		reg = (data & 0xff0000) >> 16;
		val = data & 0xffff;
		phy_write(phydev, 0x1f, page);
		phy_write(phydev, reg, val);
		if (page)
			phy_write(phydev, 0x1f, 0);
		pr_info("write page 0x%x reg 0x%02x val 0x%04x\n", page, reg, val);
	} else {
		page = (data & 0xfff00) >> 8;
		reg = data & 0xff;
		phy_write(phydev, 0x1f, page);
		...
		if (page)
			phy_write(phydev, 0x1f, 0);
		pr_info("read page 0x%x reg 0x%02x val 0x%04x\n", page, reg, val);
	}
