...
#endif
	/* For Molchip 100T EPHY */
	void (*clk_reset)(struct net_device *dev);

	__u64 (*mmc_read_status)(struct net_device *dev);
};
...
struct phy_driver {
	...
	int (*brd_link_up_cb)(struct phy_device *dev );
	int (*brd_link_down_cb)(struct phy_device *dev);
};
