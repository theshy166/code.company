...
#ifdef CONFIG_UVC_EP_TYPE_BULK
struct uvc_device *g_puvc[2];
EXPORT_SYMBOL(g_puvc);
struct usb_ep *puvc_ep[2];
EXPORT_SYMBOL(puvc_ep);

void uvc_stream_off(struct dwc3_ep *dep)
{
	struct uvc_device *uvc;
	struct v4l2_event v4l2_event;

	if (puvc_ep[0] == &dep->endpoint)
		uvc = g_puvc[0];
	if (puvc_ep[1] == &dep->endpoint)
		uvc = g_puvc[1];
	if (uvc->bulk_stream_off) {
		pr_info("[UVC BULK]%s: id %d\n",
			__func__, uvc->video.stream_id);
...
EXPORT_SYMBOL(uvc_stream_off);

static void uvc_bulk_status(unsigned int id)
{
	struct uvc_device *uvc = g_puvc[id];
	unsigned long flags;

	uvc->bulk_stream_off = 1;
}
