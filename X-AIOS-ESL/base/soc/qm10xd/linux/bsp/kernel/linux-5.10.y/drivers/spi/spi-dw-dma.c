...
static int dw_spi_dma_config_tx(struct dw_spi *dws)
{
	...
	if (dws->id == SPI_DEV0)
		...
		dev_err(&dws->master->dev, "Unknown device\n");
		return -ENODEV;
	}
	return dmaengine_slave_config(dws->txchan, &txconf);
}

static int dw_spi_dma_submit_tx(struct dw_spi *dws, dma_addr_t buf_addr,
		size_t buf_len, size_t period_len)
{
	...
	txdesc = dmaengine_prep_dma_cyclic(dws->txchan, buf_addr, buf_len,
			period_len, DMA_MEM_TO_DEV, DMA_PREP_INTERRUPT | DMA_PREP_FENCE);
...
static int dw_spi_dma_wait_rx_done(struct dw_spi *dws, struct spi_transfer *xfer)
{
	int retry = SPI_WAIT_RETRIES;
	struct spi_delay delay;
	unsigned long ns, us;
...
static int dw_spi_dma_config_rx(struct dw_spi *dws)
{
	...
	rxconf.src_maxburst = 0x1;//dws->rxburst;
...
static int dw_spi_dma_submit_rx(struct dw_spi *dws, dma_addr_t buf_addr,
		size_t buf_len, size_t period_len)
{
	...
	rxdesc = dmaengine_prep_dma_cyclic(dws->rxchan, buf_addr, buf_len,
			period_len, DMA_DEV_TO_MEM, DMA_PREP_INTERRUPT | DMA_PREP_FENCE);
				   ...
				   struct spi_transfer *xfer)
{
	...
	ret = dw_spi_dma_submit_tx(dws, virt_to_phys(xfer->tx_buf),
				xfer->len,xfer->len);
	if (ret)
		goto err_clear_dmac;
	...
	if (xfer->rx_buf) {
		ret = dw_spi_dma_submit_rx(dws, virt_to_phys(xfer->rx_buf),
				xfer->len,xfer->len);
		if (ret)
			goto err_clear_dmac;
...
static int dw_spi_dma_transfer(struct dw_spi *dws, struct spi_transfer *xfer)
{
	...
	if (xfer->rx_buf && dws->master->cur_msg->status == -EINPROGRESS)
		ret = dw_spi_dma_wait_rx_done(dws,xfer);
