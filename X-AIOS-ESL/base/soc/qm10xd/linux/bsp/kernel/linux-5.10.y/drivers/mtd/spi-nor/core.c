...
static int spi_nor_write_16bit_cr_and_check(struct spi_nor *nor, u8 cr)
{
	...
	if (sr_written != sr_cr[0]) {
		dev_dbg(nor->dev, "SR: Read back test failed\n");
		printk("SR: Read back test failed\n");
		return -EIO;
	}
	...
	if (cr != sr_cr[1]) {
		dev_dbg(nor->dev, "CR: read back test failed\n");
		printk("CR: read back test failed\n");
		return -EIO;
	}
 ...
 */
int spi_nor_erase(struct mtd_info *mtd, struct erase_info *instr)
{
 ...
 */
int spi_nor_write(struct mtd_info *mtd, loff_t to, size_t len,
...
static int spi_nor_select_erase(struct spi_nor *nor)
{
	...
	/* prefer 32K B erase if possible */
	wanted_size = 32768u;
#endif
