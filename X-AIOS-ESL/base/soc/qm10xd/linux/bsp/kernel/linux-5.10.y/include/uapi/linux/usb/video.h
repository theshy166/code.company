...
#define UVC_OUTPUT_TERMINAL_ID		5
#define UVC_MSXU_FACE_EXTENSION_UNIT_ID  7
/* --------------------------------------------------------------------------
 * UVC constants
 */
...
struct uvc_still_frame_info {
	__u16 width;
	__u16 height;
} __attribute__ ((packed));

/* still_image_frame descriptor */
#define DECLARE_UVC_STILL_IMAGE_DESCRIPTOR(n)	\
struct UVC_STILL_IMAGE_DESCRIPTOR(n) {	\
	__u8  bLength;	\
	__u8  bDescriptorType;	\
	__u8  bDescriptorSubType;	\
	__u8  bEndpointAddress;	\
	__u8  bNumImageSizePatterns;	\
	struct uvc_still_frame_info  patterns[n];	\
	__u8  bNumCompressionPattern;	\
	__u8  bCompressionPatterns[n];	\
} __attribute__((__packed__))
...
#define UVC_DT_STILL_IMAGE_SIZE(n)         (6+5*(n))
#define UVC_STILL_IMAGE_DESCRIPTOR(n)\
	uvc_still_image_frame_descriptor_##n
