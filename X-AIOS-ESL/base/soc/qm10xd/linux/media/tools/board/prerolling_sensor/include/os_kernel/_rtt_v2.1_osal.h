 ...
 *
 */
#ifndef __RTT_2_1_OSAL_H__
#define __RTT_2_1_OSAL_H__
...
HRINGBUFFER rpc_ringbuf_handle(FH_VOID);
FH_SINT32 rpc_mode_proc(FH_VOID);
#define seq_printf(fd,...) \
do{ \
	if(rpc_mode_proc()) { \
		ringbuffer_print(rpc_ringbuf_handle(),__VA_ARGS__); \
	} \
	else { \
		fh_printk(__VA_ARGS__); \
	} \
...
({ \
	unsigned char *_map = (unsigned char *)(addr); \
	IGNORE_UNUSE(size); \
	_map; \
})
...
#define fh_gettime(time_value)      *time_value = get_pts()
