...
static int getSensorConf(void *pvObj)
{
    ...
    int i;
    FH_VICAP_Get_Phy_Lane(&pstCtrlPara->u32MaxLaneNum);

    for (i = 0; i < pstCtrlPara->u32CurrConfSize; i++)
    {
        //printf("pstCtrlPara->u32SnsFmt=%d\r\n", pstCtrlPara->u32SnsFmt);
        if (pstCtrlPara->u32SnsFmt == pstConfGrp[i].u32SnsFmt)
        {
            //printf("pstCtrlPara->u32SnsFmt=%d, pstConfGrp[i].u32SnsFmt=%d\r\n", pstCtrlPara->u32SnsFmt, pstConfGrp[i].u32SnsFmt);
...
static __attribute__((unused)) FH_SINT32 SnsCom_SetFmt(FH_VOID* pvObj, FH_SINT32 s32Format, FH_UINT16 u16CciId, FH_UINT16 u8CsiMode)
{
    SNS_CTRL_PARA_S*    pstCtrlPara = _sns_get_ctrl_para(pvObj);
    SensorDevice*       pstCci      = _sns_get_cci_(pvObj);
