...
static int GetSensorViAttr(void *pvObj, ISP_VI_ATTR_S *stViAttr)
{
    ...
    switch (pstCtrlPara->u32SnsFmt)
    {
        ...
        case FORMAT_720P25:
            ...
            stViAttr->u16WndHeight = FRAM_H_720P25;
...
static void SetBlcKSun(void *pvObj)
{
    ...
    FH_UINT32 ret = Sensor_Read(pstCci, 0x0c);
    if((ret & 1) == 0)
    {
...
#ifdef OS_NONE
    SensorDevice*       pstCci      = _sns_get_cci_(pvObj);
    FH_UINT32 tmp = Sensor_Read(pstCci, 0x00);
    FH_UINT32 fine = tmp & 0xf;
    FH_UINT8 integer = (tmp & 0x70) >> 4;

    pstCtrlPara->u32CurrSnsGain = ((fine + 0x10) << integer << 2);
#endif
...
#ifdef OS_NONE
    pstCtrlPara->u32CurrSnsIntt = (Sensor_Read(pstCci,0x02) << 8) | (Sensor_Read(pstCci,0x01));
    pstCtrlPara->u32CurrSnsGain = GetGain(pvObj);
#else
...
struct isp_sensor_if *NAME(Sensor_Create)()
{
    SensorCreateInfo info = {
        .pstSnsIf       = &jxf23_dvp_sensor_if_default,
