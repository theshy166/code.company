#ifndef __K351P_DVP_500P5_H__
#define __K351P_DVP_500P5_H__

#define FRAM_W_500P5   800
#define FRAM_H_500P5   750
#define ACTIVE_W_500P5 K351P_FRAM_W_500
#define ACTIVE_H_500P5 500
#define PIC_IN_W_500P5 K351P_FRAM_W_500
#define PIC_IN_H_500P5 500
#define OFFSET_X_500P5 0
#define OFFSET_Y_500P5 0

static ISP_VI_ATTR_S attr_dvp_500P5 = {
    FRAM_H_500P5,
    FRAM_W_500P5,
    ACTIVE_H_500P5,
    ACTIVE_W_500P5,
    OFFSET_X_500P5,
    OFFSET_Y_500P5,
    PIC_IN_H_500P5,
    PIC_IN_W_500P5,
    5,
    BAYER_BGGR
};
const FH_UINT16  Sensor_Cfg_K351P_DVP_500P5[] =
{
    0x12, 0x41,
    0xAD, 0x01,
    0xAD, 0x00,
    0x0E, 0x10,
    0x0F, 0x0C,
    0x10, 0x7E,
    0x0C, 0x00,
    0x67, 0xA2,
    0x0D, 0x22,
    0x64, 0x30,
    0x65, 0xBA,
    0xBE, 0x18,
    0xBF, 0x60,
    0xBC, 0xC0,
    0x20, 0x58,
    0x21, 0x02,
    0x22, 0x34,
    0x23, 0x08,
#if K351P_FRAM_W_ALGIN
    0x24,0xe0,
#else
    0x24,0xF4,
#endif
    0x25, 0xD0,
    0x26, 0x71,
    0x27, 0x10,
    0x28, 0x11,
    0x29, 0x00,
    0x2B, 0x10,
    0x2C, 0x00,
    0x2D, 0x05,
    0x2E, 0xFB,     //2E, ,
    //[pre_ulpm]
    0x08, 0x23,
    0x6F, 0xFF,
    0xA5, 0x51,
    0xA8, 0x80,
    0xAA, 0x20,
    0xEF, 0x20,
    0xF0, 0x03,
    0xF1, 0xEE,
    //[normal_to_AO]
    0x06, 0x25,
    0x0D, 0x01,
    0x0E, 0x94,
    0x0F, 0x1C,
    //0x12, 0x01,
    0x1E, 0x7B,
    0x27, 0x06,     //27, 0x04
    0x28, 0x0D,
    0x31, 0x01,
    0x33, 0xC2,
    0x56, 0xA1,
    0x5B, 0x00,
    0x60, 0x3F,
    0x61, 0xFF,
    0x62, 0x21,
    0x78, 0x53,
    0xBA, 0x5B,
    0xBE, 0x15,
    0xBF, 0x26,
    0xEB, 0xA5,
    0xEC, 0xDB,

    //0x12, 0x01,       //12, 0x00

#if 0
    //[pre_ulpm]
    0x08, 0x23,
    0x6F, 0xFF,
    0xA5, 0x51,
    0xA8, 0x80,
    0xAA, 0x20,
    0xEF, 0x20,
    0xF0, 0x03,
    0xF1, 0xEE,

    //[normal_to_AO]
    0x06, 0x25,
    0x0D, 0x01,
    0x0E, 0x94,
    0x0F, 0x1C,
    0x12, 0x01,
    0x1E, 0x7B,
    0x27, 0x06,     //27, 0x04
    0x28, 0x0D,
    0x31, 0x01,
    0x33, 0xC2,
    0x56, 0xA1,
    0x5B, 0x00,
    0x60, 0x3F,
    0x61, 0xFF,
    0x62, 0x21,
    0x78, 0x53,
    0xBA, 0x5B,
    0xBE, 0x15,
    0xBF, 0x26,
    0xEB, 0xA5,
    0xEC, 0xDB,

    //[AO_to_normal]
    0x06, 0x00,
    0x0D, 0x22,
    0x0E, 0x10,
    0x0F, 0x0C,
    0x12, 0x00,
    0x1E, 0x73,
    0x27, 0x10,
    0x28, 0x11,
    0x31, 0x04,
    0x33, 0xCC,
    0x56, 0xF1,
    0x5B, 0x10,
    0x60, 0x60,
    0x61, 0x00,
    0x62, 0x60,
    0x78, 0x49,
    0xBA, 0x8B,
    0xBE, 0x18,
    0xBF, 0x60,
    0xEB, 0x15,
    0xEC, 0x03,
#endif
};

#endif  // __K351P_DVP_500P5_H__
