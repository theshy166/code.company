...
#else
#include "bare_i2c_api.h"
#include "bare_i2c.h"
#define _IOW(a, cmd, c) cmd
typedef int SEN_DEV_FD;
static iic_drv_t iic_dev = {0};
extern int iic_write(iic_drv_t *instance, unsigned int addr, unsigned int data);
extern unsigned int iic_read(iic_drv_t *instance, unsigned int addr);
#endif
...
#ifdef OS_LINUX
#define ISP_DEV_NAME        "/dev/isp460"
#else
#define ISP_DEV_NAME        "ISP460"
#endif
// static SensorDevice sdev = {0};
static SEN_DEV_FD sensor_isp_device;
...
#define SENSOR_OPS_FUN_DEBUG 0
#define SENSOR_OPS_DEBUG 0
...
void SPISensor_Write(SensorDevice *sdev, FH_UINT16 addr, FH_UINT32 data)
{

}

FH_UINT16 SPISensor_Read(SensorDevice *sdev, FH_UINT16 addr)
{
	return 0;
}
#ifdef OS_RTT
rt_err_t I2CSensor_WriteEx(SensorDevice *sdev, struct rt_i2c_bus_device *fh81_i2c,rt_uint16_t reg, rt_uint16_t data, int no_ack)
{
...
rt_err_t I2CSensor_WriteEx_Multi(SensorDevice *sdev, struct rt_i2c_bus_device *fh81_i2c,rt_uint16_t *reg, rt_uint16_t *data, int no_ack,int size)
{
	struct rt_i2c_msg *msg;
...
#elif OS_LINUX
void I2CSensor_WriteEx(SensorDevice *sdev, FH_UINT16 addr, FH_UINT16 data, int no_ack)
{
...
void I2CSensor_WriteEx_Multi(SensorDevice *sdev, FH_UINT16 *addr, FH_UINT16 *data, int no_ack,int size)
{
	struct i2c_rdwr_ioctl_data i2c_data;
	FH_UINT8 *tmp;
	int ret;
...
#elif OS_NONE
#if 0
int I2CSensor_WriteEx(FH_UINT16 addr, FH_UINT16 data, int no_ack)
{
	int ret = 0;
...
int I2CSensor_WriteEx_Multi(FH_UINT16 *addr, FH_UINT16 *data, int no_ack,int size)
{
	int ret = 0;
	int i;
...
void I2CSensor_Write_Multi(SensorDevice *sdev, FH_UINT16 *addr, FH_UINT16 *data,FH_UINT16 size)
{
#ifdef OS_RTT
...
#endif
void I2CSensor_Write(SensorDevice *sdev, FH_UINT16 addr, FH_UINT16 data)
{
#ifdef OS_RTT
...
#ifdef OS_RTT
rt_err_t I2CSensor_Read(SensorDevice *sdev, struct rt_i2c_bus_device *fh81_i2c,rt_uint16_t reg, rt_uint16_t *data)
{
...
#elif OS_LINUX
FH_UINT16 I2CSensor_Read(SensorDevice *sdev, FH_UINT16 addr)
{
	struct i2c_rdwr_ioctl_data i2c_data;
...
#ifdef FUN_EN_WRITE_MULTI
void Sensor_Write_Multi(SensorDevice *sdev, FH_UINT16* addr, FH_UINT16* data,FH_UINT16 size)
{
	if(sdev->mode > 3 )
...
void Sensor_WriteEx(SensorDevice *sdev, FH_UINT16 addr, FH_UINT16 data, int no_ack)
{
	if(sdev->mode > 3 )
...
int SensorDevice_Init(SensorDevice *sdev,FH_UINT16 i2c_devid, FH_UINT8 mode, FH_UINT32 devIdx)
{
	...
	iic_dev.slaveId = i2c_devid;

	#if SENSOR_OPS_DEBUG
	fh_printf("i2c_dev_addr :%d\n", iic_dev.slaveId);
...
int SensorDevice_Close(SensorDevice *sdev)
{
	...
	// tbd
	fh_memset((void *)sensorContext, 0x00, sizeof(sensor_ops_context_t));
