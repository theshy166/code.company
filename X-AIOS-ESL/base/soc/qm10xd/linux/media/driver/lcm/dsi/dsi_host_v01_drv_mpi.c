...
#define DSIH_FIFO_ACTIVE_WAIT      100   //(10000000)    /* no of tries to access the fifo */
#define DSIH_FIFO_ACTIVE_WAIT_TIMEOUT_4_IDENTIFY    100     //(20000)   /* no of tries to access the fifo */
...
//    coreContext->tx_esc_clk_division = 0xc;
    ...
    #if DSI_HOST_MPI_DEBUG
    printk("coreContext->host_sync_div   : %d\n", host_sync_div              );
    printk("coreContext->vid_pkt_size    : %d\n", coreContext->vid_pkt_size  );
    printk("coreContext->vid_hsa_time    : %d\n", coreContext->vid_hsa_time  );
    printk("coreContext->vid_hbp_time    : %d\n", coreContext->vid_hbp_time  );
    printk("coreContext->vid_hline_time  : %d\n", coreContext->vid_hline_time);
    printk("coreContext->vsa_lines       : %d\n", coreContext->vsa_lines     );
    printk("coreContext->vbp_lines       : %d\n", coreContext->vbp_lines     );
    printk("coreContext->vfp_lines       : %d\n", coreContext->vfp_lines     );
    printk("coreContext->v_active_lines  : %d\n", coreContext->v_active_lines);

    printk("dp_sync_info->hor_sync_active          : %d\n", dp_sync_info->hor_sync_active );
    printk("dp_sync_info->hor_back_porch           : %d\n", dp_sync_info->hor_back_porch  );
    printk("dp_sync_info->hor_active_pixel         : %d\n", dp_sync_info->hor_active_pixel);
    printk("dp_sync_info->hor_front_porch          : %d\n", dp_sync_info->hor_front_porch );
    printk("dp_sync_info->ver_sync_active          : %d\n", dp_sync_info->ver_sync_active );
    printk("dp_sync_info->ver_back_porch           : %d\n", dp_sync_info->ver_back_porch  );
    printk("dp_sync_info->ver_active_line          : %d\n", dp_sync_info->ver_active_line );
    printk("dp_sync_info->ver_front_porch          : %d\n", dp_sync_info->ver_front_porch );
    #endif
...
static int32_t _dsi_gen_rd_packet_4_identify(host_core_t hostId, uint8_t vc, uint8_t data_type, uint8_t msb_byte,
    uint8_t lsb_byte, uint8_t bytes_to_read, uint8_t* read_buffer)
{
    int32_t rtn = RTN_DSI_SUCCESS;
    ...
    for (timeout = 0; timeout < DSIH_FIFO_ACTIVE_WAIT_TIMEOUT_4_IDENTIFY; timeout++) {
        /* check if payload Tx fifo is not full */
        if (!_dsi_hal_gen_cmd_fifo_full(hostId)) {
            _dsi_hal_gen_packet_header(hostId, vc, data_type, msb_byte, lsb_byte);
            dsi0_udelay(100);
    ...
    if (!(timeout < DSIH_FIFO_ACTIVE_WAIT_TIMEOUT_4_IDENTIFY)) {
        printf("dsi: tx rd command timed out\n");
        return 0;
    }
    /* loop for the number of words to be read */
...
uint32_t _dsi_dcs_rd_cmd_4_identify(host_core_t hostId, uint8_t vc, uint8_t data_type, uint8_t command, uint8_t bytes_to_read, uint8_t* read_buffer)
{
    switch (data_type)
    {
        case 0xA8:
        ...
        case 0x06:
            /* COMMAND_TYPE 0x06 - DCS Read no params refer to DSI spec p.47 */
            return _dsi_gen_rd_packet_4_identify(hostId, vc, 0x06, 0x0, command, bytes_to_read, read_buffer);
        default:
            printf("dsiMipi: invalid DCS command");
            return 0;
    }
...
int32_t dsi_dcs_rd_cmd_4_identify(host_core_t hostId, uint8_t data_type, uint8_t command, uint8_t bytes_to_read, uint8_t *read_buffer)
{
    ...
    rtn = _dsi_dcs_rd_cmd_4_identify(hostId, 0, data_type, command, bytes_to_read, read_buffer);
    EXIT_IF_FAIL(0x00 == rtn, NULL, "_dsi_dcs_rd_cmd ~_~\n");
	/* fix esl double screen identify bug */
    if((command == 0x0a && data_type == 0x06 && rtn != 0) || (command == 0x04 && data_type == 0x06 && rtn != 0)){
        goto EXIT;
    }
