...
typedef struct
{
	...
	FH_UINT8         data_clip_min; //!< 输出数据最小值限制。取值范围: [0-255]
	FH_UINT8         data_clip_max; //!< 输出数据最大值限制。取值范围: [0-255]
...
typedef struct
{
	...
	FH_UINT16    gau_coeff0;    //!< 高斯滤波器系数。取值范围: [0-511]
	FH_UINT8     gau_coeff1;    //!< 高斯滤波器系数。取值范围: [0-63]
	FH_UINT8     gau_coeff2;    //!< 高斯滤波器系数。取值范围: [0-63]
	FH_UINT8     gau_coeff3;    //!< 高斯滤波器系数。取值范围: [0-63]
	FH_UINT8     gau_coeff4;    //!< 高斯滤波器系数。取值范围: [0-63]
	FH_UINT8     gau_coeff5;    //!< 高斯滤波器系数。取值范围: [0-63]
	FH_UINT8     data_clip_min; //!< 输出数据最小值限制。取值范围: [0-255]
	FH_UINT8     data_clip_max; //!< 输出数据最大值限制。取值范围: [0-255]
	IVE_IMAGE    src_addr;      //!< 输入图像地址
	IVE_IMAGE    dst_addr;      //!< 输出图像地址
...
typedef struct
{
	...
	FH_IVE_COMP      srccomp;       //!< Y/U/V。取值范围: [0-2]
	FH_IVE_PIX_WIDTH srcwidth;      //!< 输入位宽。取值范围: [0-5]
	FH_ROTATE_OPS    rotate;        //!< 旋转角度。取值范围: [0-3]
...
typedef struct
{
	...
	FH_UINT8     data_clip_min;     //!< 输出数据最小值限制。取值范围: [0-255]
	FH_UINT8     data_clip_max;     //!< 输出数据最大值限制。取值范围: [0-255]
	FH_UINT32    type;              //!< 输入格式 0:Y 1:ARGB 2:NV16 3:NV12。取值范围: [0-3]
