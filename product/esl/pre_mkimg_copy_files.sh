...
function xz_decompress()
{
	...
	FAKEROOT_PATH=$(PATH=".:$PATH" which fakeroot)
        ...
        "$FAKEROOT_PATH" xz -d -c $SRC_FILE_ROOTFS | "$FAKEROOT_PATH" cpio -id
...
if [ -d "$DES_OUT_ROOT_DIR/../xos/res/h5esl" ];then
cp -arf $DES_OUT_ROOT_DIR/../xos/res/h5esl $DES_OUT_RES_DIR
fi
#
#
