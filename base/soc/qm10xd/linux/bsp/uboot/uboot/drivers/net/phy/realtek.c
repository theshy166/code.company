...
#define MIIM_RTL8201F_PAGE_SELECT      0x1f
...
static int rtl8201f_config(struct phy_device *phydev)
{
	...
	if ((reg & BMCR_ANENABLE) && !(reg & BMCR_RESET))
		goto complete_reset;

another_reset:
	if (phy_write(phydev, MDIO_DEVAD_NONE, MII_BMCR, BMCR_RESET) < 0) {
...
complete_reset:
	phy_write(phydev, MDIO_DEVAD_NONE,
		  MIIM_RTL8201F_PAGE_SELECT, 0x4);
	phy_write(phydev, MDIO_DEVAD_NONE, 16, 0);

	phy_write(phydev, MDIO_DEVAD_NONE,
		  MIIM_RTL8201F_PAGE_SELECT, 7);
