...
#define aes_dma_read(reg)					raw_readl((unsigned long)(REG_DMA_BASE + (reg)))
#define aes_dma_write(val,reg)				raw_writel(val,(unsigned long)(REG_DMA_BASE + (reg)))

#define DMA_WAIT_READY_TIME_OUT		    1000

extern void enable_channel(u32 chn);
extern void aes_dma_trac(unsigned int src, unsigned int dest,
	unsigned int chn, unsigned int req_id, unsigned int len, bool fix);
extern void aes_enable_channel(u32 chn);
#endif
