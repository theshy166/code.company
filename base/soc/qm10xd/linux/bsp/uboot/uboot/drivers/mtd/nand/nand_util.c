		...
		u_char *buffer)
{
	...
	oobsize = mtd->oobsize;
    printf("%s: offset = 0x%llx, length = %d, writesize = %d, oobsize = %d\n",
            __func__, offset, *length, mtd->writesize, oobsize);
	...
	if (len_incl_bad == length_data) {
		for (i = 0; i < length_data / mtd->writesize; i++) {
			...
			ops.ooblen = oobsize;
			ops.mode = MTD_OPS_PLACE_OOB;//MTD_OPS_RAW;
		    ...
		    u_char *buffer)
{
	...
	if ((*length % (mtd->writesize  + oobsize)) != 0) {
		printf("Attempt to read non aligned data, read length should "\
				"be aligned with (pagesize + oobsize), length:%ld "
				"pagesize:%d oobsize:%d\n",
				(long int)*length, mtd->writesize, oobsize);
		return -EINVAL;
	}
