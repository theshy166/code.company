...
#define		UART0_REG_BASE			(0x13030000)
//#define		UART1_REG_BASE			(0x13030000)
 ...
 ***************************************************************************/
UartDesc l_uart={CONSOLE_PORT,BAUDRATE_115200,Uart_line_8n1,UART_INT_RXFIFO_DEPTH_QUARTER};                                 //UART struct
...
void fh_serial_setbrg(void)
{

}

static UINT8 uart_init_notReady = 1;
int fh_serial_init(void)
{
	l_uart.port = CONSOLE_PORT;
	l_uart.baudrate = BAUDRATE_115200;
	l_uart.line_ctrl = Uart_line_8n1;
	l_uart.fifo_ctrl = UART_INT_RXFIFO_DEPTH_QUARTER;
	Uart_Init(&l_uart);
    uart_init_notReady = 0;
	return 0;
}

int fh_serial_getc(void)
{
	UINT8 ret_data;
    if(uart_init_notReady==1)
        fh_serial_init();
	Uart_Read_Data(&l_uart, &ret_data, 1);
	return (int) ret_data;
}

void fh_serial_putc(const char c)
{
    if(uart_init_notReady==1)
        fh_serial_init();
	Uart_Write_Data(&l_uart, (UINT8 *) &c, 1);
	/* If \n, also do \r */
	if (c == '\n')
		serial_putc('\r');
}

int fh_serial_tstc(void)
{
	UINT8 ret;
    if(uart_init_notReady==1)
        fh_serial_init();
	ret = (UINT8) Uart_Read_Line_Status(&l_uart);
	return ((ret & UART_LINE_STATUS_DR) != 0);
}

void fh_serial_puts(const char *s)
{
    if(uart_init_notReady==1)
        fh_serial_init();
	while (*(s) != '\0')
...
static struct serial_device fh_serial_drv = {
	.name	= "fh_serial",
	.start	= fh_serial_init,
	.stop	= NULL,
	.setbrg	= fh_serial_setbrg,
	.putc	= fh_serial_putc,
...
void fh_serial_initialize(void)
{
    // fh_serial_init();

	serial_register(&fh_serial_drv);
}

__weak struct serial_device *default_serial_console(void)
{
	return &fh_serial_drv;
}
