...
#define dma_read(reg)			__raw_readl(REG_DMA0_BASE + (reg))
#define dma_write(val, reg)		__raw_writel(val, REG_DMA0_BASE + (reg))
...
typedef struct _dma_link_list_dev {
	...
	u32 word_mode;
	Direction direction;
	u32 src_addr_arr[MAX_NODE];
	u32 dst_addr_arr[MAX_NODE];
	u32 trsc_len_arr[MAX_NODE];
	u32 node_cnt;
} dma_link_list_dev;
...
void dma_chn_src_high(u32 chn, u32 src);
...
void dma_chn_priority(u32 chn, u32 pri);
void dma_req_mode(u32 chn, u32 mode);
void dma_switch_mode(u32 chn, u32 mode);
void dma_ahb_size(u32 chn, u32 size);
void dma_trsf_step(u32 chn, u32 step);
void dma_clk_enable(void);
void dma_frag_wait(u32 wait);
void dma_fix_enable(u32 chn, bool en);
void dma_wrap_enable(u32 chn, bool en);
void hash_request_cid(u32 chn);
void spic_request_cid(u32 chn, u32 rid);
void clear_dma_reg(u32 chn);
void set_llist_ptr(u32 chn, u32 ptr);
void sfc_node_fill(u8 *dest);
void dma_init(void);
void hash_dma_init(u32 chn);
void dma_init_chnel(u32 chn, u32 rid);
void sfc_dma_node_fill(u8 *dest);
void dma_link_list(dma_link_list_dev *dma_dev, u32 loop, NodeCfg *node);
void llist_enable(u32 chn);
void llist_valid(u32 chn);
void llist_node_last(u32 chn, bool last);
extern void aes_request_cid(u32 chn, unsigned int req_id);
void hash_dma_trsc_len(u32 chn, u32 len);

void aes_dma_node_fill(unsigned int src, unsigned int dest,
	unsigned int chn, unsigned int req_id, unsigned int sel);
