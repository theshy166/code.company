...
int fdt_chosen(void *fdt)
{
	...
	if (str) {
		...
		printf("Can't get bootargs from env\n");
	}
...
int fdt_fixup_cpu_freq(void *blob, u32 cpu_freq)
{
	int err, nodeoffset;
        ...
        /* find or create "/timer" node. */
        nodeoffset = fdt_find_or_add_subnode(blob, 0, "timer");
        if (nodeoffset < 0)
                        return nodeoffset;

        err = fdt_setprop_u32(blob, nodeoffset, "clock-frequency", cpu_freq);
        if (err < 0) {
                return err;
        }
