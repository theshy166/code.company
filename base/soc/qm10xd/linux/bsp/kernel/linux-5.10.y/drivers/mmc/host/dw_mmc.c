...
static void dw_mci_setup_bus(struct dw_mci_slot *slot, bool force_clkinit)
{
	...
	if (!clock) {
		...
		if (!test_bit(DW_MMC_CARD_NO_LOW_PWR, &slot->flags)) {
			#ifdef CONFIG_PROJ_XKPHOTO
			// sd_id == 0 is for sdio wifi, low_power decrease performace xkphoto
			printk("host->sd_id not set lowpower mode\n");
			//if( host->sd_id != 0) {
			//	clk_en_a |= SDMMC_CLKEN_LOW_PWR << slot->id;
			//}
			#elif defined(CONFIG_PROJ_ESL)
			// sd_id == 1 is for sdio wifi, low_power decrease performace esl
			printk("host->sd_id not set lowpower mode\n");
			// if( host->sd_id != 1) {
			// 	clk_en_a |= SDMMC_CLKEN_LOW_PWR << slot->id;
			// }
...
static struct dw_mci_board *dw_mci_parse_dt(struct dw_mci *host)
{
	...
	device_property_read_u32(host->dev, "cd-detect-level", &host->cd_detect_level);
#if CONFIG_PM_SLEEP
	device_property_read_u32(host->dev, "waitting-time-suspend", &host->suspend_wtime);
...
static void mc_sd_pll_cfg(struct dw_mci *host)
{
	unsigned int reg_INIT_CFG,reg_DLL_SEL;
	unsigned int val = 0;
	void *addr;

	if(host->sd_id==0) {
		addr = ioremap(CPU_SYS_AHB_BASE,0x1000);
		reg_INIT_CFG = SDIO0_SPL1_REG;
		reg_DLL_SEL = SDIO0_CTRL0_REG;
	} else if(host->sd_id==1) {
		addr = ioremap(DMC_SYS_AHB_BASE,0x1000);
		reg_INIT_CFG = SDIO1_DLLININFO_SPL_REG;
		reg_DLL_SEL = SDIO1_CTRL0_REG;
	}

	val = readl_relaxed(addr + reg_DLL_SEL);
	val &= ~(SDIO_DLLBYPASS_VALUE | SDIO_DLLCLKSEL_VALUE);
	writel_relaxed(val,addr + reg_DLL_SEL);

	writel_relaxed(SDIO_DLLININFO_VALUE,addr + reg_INIT_CFG);
	iounmap(addr);
}


static int mc_sd_clk_cfg(struct dw_mci *host)
{
	if (host->sd_id == 0) {
		host->ckg_mux_num = CKG_MUX_SDIO0;
		host->ckg_dllref_num = CKG_SDIO0_DLLREF_EN;
		host->ckg_ahp_sdio_num = CKG_AHB_SDIO0_EN;
		host->ckg_sdio_num = CKG_SDIO0_EN;
	}
	else if (host->sd_id == 1) {
		host->ckg_mux_num = CKG_MUX_SDIO1;
		host->ckg_dllref_num = CKG_SDIO1_DLLREF_EN;
		host->ckg_ahp_sdio_num = CKG_AHB_SDIO1_EN;
		host->ckg_sdio_num = CKG_SDIO1_EN;
	} else {
		return -ENODEV;
	}
...
int dw_mci_probe(struct dw_mci *host)
{
	...
	device_property_read_u32(host->dev, "id", &host->sd_id);
	mc_sd_pll_cfg(host);
	ret = mc_sd_clk_cfg(host);
	if (ret){
		dev_err(host->dev, "Unknown SDIO Device Id To Config Clk \n");
		return ret;
	}
	ret = clk_mini_set_rate(host->ckg_mux_num, host->pdata->bus_hz);
	if (ret){
		dev_err(host->dev, "failed to set rate %uHz\n", host->pdata->bus_hz);
		return ret;
	}
	/* The clock interface is not yet available */
	host->bus_hz = clk_mini_get_rate(host->ckg_mux_num);
	ret = clk_mini_enable(host->ckg_dllref_num);
	if (ret) {
		dev_err(host->dev, "set dllref failed \n");
		return ret;
	}
	ret = clk_mini_enable(host->ckg_ahp_sdio_num);
	if (ret) {
		dev_err(host->dev, "set biu failed \n");
		return ret;
	}
	ret = clk_mini_enable(host->ckg_sdio_num);
	if (ret) {
		dev_err(host->dev, "set ciu failed \n");
		goto err_clk_biu;
	}
...
err_clk_ciu:
	clk_mini_disable(host->ckg_sdio_num);

err_clk_biu:
	clk_mini_disable(host->ckg_ahp_sdio_num);
...
/*
	Reserved power management interface
	1:power on
	0:power down
*/
int sdio_power_down_up(int mode)
{
	return 0;
}

int mc_sd_resume(struct device *dev)
{
	int ret = 0;
	struct dw_mci *host = dev_get_drvdata(dev);

	sdio_power_down_up(1);
	...
	/* The clock interface is not yet available */
	host->bus_hz = host->pdata->bus_hz;//clk_mini_get_rate(host->ckg_mux_num);
	ret = clk_mini_enable(host->ckg_dllref_num);
	if (ret) {
		dev_err(host->dev, "set dllref failed \n");
		return ret;
...
int mc_sd_suspend(struct device *dev)
{
	struct dw_mci *host = dev_get_drvdata(dev);
	struct mmc_host	*prev_mmc = host->slot->mmc;
	int timeout=500;
	...
	if(host->suspend_wtime)
		timeout = host->suspend_wtime;

	if(host->force_suspend)
		force_suspend = host->force_suspend;

	while(dw_mci_card_busy(prev_mmc)){
		timeout--;
		if (timeout == 0){
			printk("sdio suspend timeout \n");
			if(force_suspend)
				goto out;
			return 0;
		}
...
out:
	sdio_power_down_up(0);

	clk_mini_disable(host->ckg_sdio_num);
	clk_mini_disable(host->ckg_ahp_sdio_num);
	clk_mini_disable(host->ckg_dllref_num);	/* dll configs clk, not affected others*/
