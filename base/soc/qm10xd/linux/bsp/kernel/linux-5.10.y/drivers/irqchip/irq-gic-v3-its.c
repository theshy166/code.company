...
static unsigned long long gits_translater = 0ULL;
...
static u64 its_irq_get_msi_base(struct its_device *its_dev)
{
	...
	if (its->translater_addr == 0ULL)
		return its->phys_base + GITS_TRANSLATER;
	else
		return its->translater_addr;
}
				...
				struct fwnode_handle *handle, int numa_node)
{
	...
	if (gits_translater == 0ULL)
		its->translater_addr = 0ULL;
	else
		its->translater_addr = gits_translater;
...
static int __init its_of_probe(struct device_node *node)
{
	...
	for (np = of_find_matching_node(node, its_device_id); np;
		...
		ret = of_property_read_u64(np, "gits-translater", &gits_translater);
		if (ret) {
			pr_warn("pcie: gits-translater not found in %s", np->full_name);
			gits_translater = 0ULL;
		}
