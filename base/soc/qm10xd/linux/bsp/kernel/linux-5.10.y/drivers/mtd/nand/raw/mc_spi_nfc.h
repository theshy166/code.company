...
#define MC_WAIT_TIMEOUT			(400000)  /* 4s = 400000*10us */
#define MC_CHIP_DELAY				25
#define MC_MAX_NAND_CHIP_NUM           2
...
struct mc_snfc_host {
	struct mtd_info *mtd;
	struct nand_chip *chip[MC_MAX_NAND_CHIP_NUM];
...
#endif
	void (*send_cmd_pageprog)(struct mc_snfc_host *host, struct nand_chip *chip, 
		struct mtd_info *mtd,  const uint8_t *buf, int oob_required);
	void (*send_cmd_status)(struct mc_snfc_host *host);
	void (*send_cmd_readstart)(struct mc_snfc_host *host, struct nand_chip *chip);
