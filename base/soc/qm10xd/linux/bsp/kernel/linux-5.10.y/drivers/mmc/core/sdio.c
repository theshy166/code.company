...
for wifi scan card
*/
void mc_sdio_card_scan(int sd_id)
{
	char name[32];
	...
	sprintf(name, "dwmmc%d", sd_id);
	np = of_find_node_by_name(NULL, name);
	if (np && !IS_ERR(np))
		pdev = of_find_device_by_node(np);
	else{
		if(sd_id)
			sprintf(name, "1d300000.dwmmc%d", sd_id);
		else
			sprintf(name, "1d400000.dwmmc%d", sd_id);
		dev = bus_find_device_by_name(&platform_bus_type, NULL, name);
		if (dev && !IS_ERR(dev))
			pdev = to_platform_device(dev);
		else
			pr_err("%s:%d, %s dev not fount\n", __FUNCTION__, __LINE__, name);
	}

	if(!pdev){
		pr_err("%s:%d, device not fount\n", __FUNCTION__, __LINE__);
		return;
	}
	host = platform_get_drvdata(pdev);
	if (!host) {
		pr_err("%s:%d, host not fount\n", __FUNCTION__, __LINE__);
		return;
	} else {
		slot = host->slot;
		slot->mmc->caps &= ~MMC_CAP_NEEDS_POLL;
		slot->mmc->caps &= ~MMC_CAP_NONREMOVABLE;
...
EXPORT_SYMBOL(mc_sdio_card_scan);
