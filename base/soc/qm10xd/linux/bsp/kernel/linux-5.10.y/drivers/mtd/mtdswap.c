...
static int mtdswap_read_markers(struct mtdswap_dev *d, struct swap_eb *eb)
{
	...
	ops.ooboffs = 0;
	// ops.datbuf = NULL;
	ops.datbuf = d->page_buf;  // 设置数据缓冲区
	ops.len = 2 * (d->mtd->writesize); // 设置读入长度
				...
				u16 marker)
{
	...
	struct mtd_oob_ops ops;
	// 准备全FF数据缓冲区
	memset(d->page_buf, 0xFF, d->mtd->writesize);
	ops.ooboffs = 0;
	ops.oobbuf = (uint8_t *)&n;
	ops.mode = MTD_OPS_AUTO_OOB;
	ops.datbuf = d->page_buf;  // 设置数据缓冲区
	ops.len = d->mtd->writesize; // 设置写入长度
