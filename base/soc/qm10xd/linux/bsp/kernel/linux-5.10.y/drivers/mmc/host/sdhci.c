	...
	struct mmc_data *data, int sg_count)
{
	...
	void *desc, *align;
	char *buffer;
	int len, offset, i, split_len;
	dma_addr_t end_addr, coalescing;
	...
	for_each_sg(data->sg, sg, host->sg_count, i) {
		...
		if (len) {
			end_addr = addr + len;
			coalescing = end_addr & (~0x7FFFFFFLL);
			if ((addr < coalescing) && (coalescing < end_addr)) {
				split_len = end_addr & 0x7FFFFFF;
				len -= split_len;
				__sdhci_adma_write_desc(host, &desc, addr, len,
							ADMA2_TRAN_VALID);
				addr += len;
				__sdhci_adma_write_desc(host, &desc, addr, split_len,
							ADMA2_TRAN_VALID);
			} else {
			/* tran, valid */
				__sdhci_adma_write_desc(host, &desc, addr, len,
