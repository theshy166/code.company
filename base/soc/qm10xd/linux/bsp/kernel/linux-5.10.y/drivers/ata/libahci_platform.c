						   ...
						   unsigned int flags)
{
	...
	hpriv->comb_phy_cfg = devm_ioremap_resource(dev,
			      platform_get_resource(pdev, IORESOURCE_MEM, 1));
	if (IS_ERR(hpriv->comb_phy_cfg)) {
		dev_err(dev, "no comb phy cfg space\n");
		rc = PTR_ERR(hpriv->comb_phy_cfg);
		goto err_out;
	}
	/*add by zhoudapeng vdu_ahb_mmio init*/
	hpriv->vdu_ahb_mmio = syscon_regmap_lookup_by_phandle(dev->of_node,
								"molchip,syscon-ap-vdu");
	if(IS_ERR(hpriv->vdu_ahb_mmio)){
		dev_err(dev,"no vdu_ahb_mmio space\n");
		rc = PTR_ERR(hpriv->vdu_ahb_mmio);
		goto err_out;
	}

	hpriv->cen_apb_mmio = syscon_regmap_lookup_by_phandle(dev->of_node,
								"molchip,syscon-cen-apb");
	if(IS_ERR(hpriv->cen_apb_mmio)){
		dev_err(dev,"no cen_apb_mmio space\n");
		rc = PTR_ERR(hpriv->cen_apb_mmio);
		goto err_out;
	}
