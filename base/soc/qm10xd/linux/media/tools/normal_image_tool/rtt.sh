...
function handle()
{
	...
	echo "24:${org_h_filesize:6:2}" | xxd -r - ${header}
	echo "25:${org_h_filesize:4:2}" | xxd -r - ${header}
	echo "26:${org_h_filesize:2:2}" | xxd -r - ${header}
	echo "27:${org_h_filesize:0:2}" | xxd -r - ${header}
	
	echo "1c:${src_base:6:2}" | xxd -r - ${header}
	echo "1d:${src_base:4:2}" | xxd -r - ${header}
	echo "1e:${src_base:2:2}" | xxd -r - ${header}
	echo "1f:${src_base:0:2}" | xxd -r - ${header}
	
	echo "20:${RTT1_BASE:6:2}" | xxd -r - ${header}
	echo "21:${RTT1_BASE:4:2}" | xxd -r - ${header}
	echo "22:${RTT1_BASE:2:2}" | xxd -r - ${header}
	echo "23:${RTT1_BASE:0:2}" | xxd -r - ${header}
