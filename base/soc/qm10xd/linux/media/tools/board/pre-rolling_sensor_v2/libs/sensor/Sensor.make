...
ifneq (,$(filter $(BUILD_MODE),6))
BARE_TARGET = $(LIBPATH)/lib$(SENSOR_NAME)_osnone.a
TARGET_BIN = $(BARE_TARGET)
endif
...
EXTRA_CFLAGS += -I$(ISP_ROOT)/include/isp \
		...
		-I$(DSP_ROOT)/include
ifeq ($(LINUX_PRE),1)
EXTRA_CFLAGS +=-I$(TOP_DIR)/sensor_common
endif

ifeq ($(CHIP_ID), XC01)
EXTRA_CFLAGS += -I$(ISP_ROOT)/drivers/fh_std/include
EXTRA_CFLAGS += -I$(ISP_ROOT)/drivers/fh_std/os_kernel
EXTRA_CFLAGS += -I$(ISP_ROOT)/drivers/fh_std/os_user
EXTRA_CFLAGS += -I$(DSP_ROOT)/media_process
EXTRA_CFLAGS += -I$(MEDIA_LIB)/include/dsp/
EXTRA_CFLAGS += -I$(MEDIA_LIB)/include/
EXTRA_CFLAGS += -I$(MEDIA_LIB)/drv_include/
endif
...
ifeq ($(CHIP_ID),XC01)
CFLAGS += -I$(RPC_ROOT)/
CFLAGS += -I$(RPC_ROOT)/include
CFLAGS += -I$(RPC_ROOT)/include/libc
CFLAGS += -I$(RPC_ROOT)/bsp/${CHIP_ID}
CFLAGS += -I$(RPC_ROOT)/bsp/${CHIP_ID}/driver
CFLAGS += -I$(RPC_ROOT)/components/finsh
CFLAGS += -I$(RPC_ROOT)/components/i2c
CFLAGS += -I$(RPC_ROOT)/libcpu/arc/arc600
endif
...
#############for OSNONE############################################
ifneq ($(BARE_TARGET),)
CC = $(OS_NONE_CROSS_COMPILE)gcc
AR = $(OS_NONE_CROSS_COMPILE)ar
EXTRA_CFLAGS+=$(RTT_EXTRA_CFLAGS)
EXTRA_CFLAGS+=-D${CHIP_ID}

ifeq ($(OS_NONE),1)
export OS_NONE_CROSS_COMPILE=$(RTT_ARC_CROSS_COMPILE)
export ARCH=ARC
...
CFLAGS+= -mno-sdata -mA6 -mmul32x16 -mno-volatile-cache -ffunction-sections
AFLAGS = ' -c  -mno-sdata -mA6 -x assembler-with-cpp'
