 ...
 *
 */
#ifndef __LINUX_4_9_OSAL_H__
#define __LINUX_4_9_OSAL_H__
...
/*clk*/
//Todo:linux49 clk-tree尚未实现,此处暂不调用
typedef struct clk                  fh_clock;
#define fh_clk_get(name)            (fh_clock *)clk_get(NULL,name)
#define fh_clk_put(clk)             clk_put(clk)
// #define fh_clk_enable(clk)          if(!fh_clk_is_enabled(clk)){fh_clk_prepare_enable(clk);}
// #define fh_clk_disable(clk)         if(fh_clk_is_enabled(clk)){fh_clk_disable_unprepare(clk);}
#define fh_clk_is_err(clk)          IS_ERR(clk)
#define fh_clk_set_rate(clk,rate)   fh_clk_set_rate(clk,rate)
#define fh_clk_get_rate(clk)        fh_clk_get_rate(clk)

// only for linux49
#define fh_irq_map(domain,hwirq) fh_irq_create_mapping(domain,hwirq)
#define fh_irq_map_ex(domain,hwirq,add) fh_irq_create_mapping(domain,hwirq)
