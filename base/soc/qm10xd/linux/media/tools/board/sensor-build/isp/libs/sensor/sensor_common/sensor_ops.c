...
static char *get_sensor_version(void)
{
...
#else
		rt_kprintf("Error: ISP device open failed!\n");
#endif
	else
	{
...
#ifdef OS_RTT
static FH_UINT16 SRead(SensorDevice *sdev, FH_UINT16 addr)
{
		I2CSensor_Read(sdev, sdev->fh81_i2c,addr,&data);
		rt_kprintf("data=%x\n",data);
	return data;
...
static FH_UINT16 SWrite(SensorDevice *sdev, FH_UINT16 addr,FH_UINT16 data)
{
		I2CSensor_WriteEx(sdev, sdev->fh81_i2c,addr, data, 0);
		rt_kprintf("write ok\n");
	return 0;
...
int SensorDevice_Init(SensorDevice *sdev, FH_UINT16 i2c_devid, FH_UINT8 mode, FH_UINT8 deviceId)
{
    ...
    if(deviceId==0)
        ...
        sdev->fh81_i2c=rt_i2c_bus_device_find("i2c1");
    else if(deviceId==2)
        sdev->fh81_i2c=rt_i2c_bus_device_find("i2c2");
	else if(deviceId==3)
        sdev->fh81_i2c=rt_i2c_bus_device_find("i2c3");
    if( sdev->fh81_i2c == NULL){
        rt_kprintf("Error: Sensor device(%d) OutofRange!\n",deviceId);
    }
