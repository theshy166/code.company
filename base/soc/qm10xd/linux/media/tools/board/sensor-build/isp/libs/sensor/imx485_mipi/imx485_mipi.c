...
#include "mipi_api.h"
#include "sensor.h"
#include "imx485_mipi.h"
#include "sensor_std.h"

#define _NAME(n, s) n##_##s
#define NAME(n) _NAME(n, imx485_mipi)
...
extern const unsigned int IMX485RegToGain[241];
...
static int SetGain(void* pvObj, FH_UINT32 again, FH_UINT8 fNo)
{
    ...
    for(i=0; i<241; i++)
    {
        if(IMX485RegToGain[i] > again)
            break;
    }
    gain_val = (i - 1);

    // err_sum = again - IMX485RegToGain[gain_val];
    pstCtrlPara->u32CurrSnsGain = IMX485RegToGain[gain_val];
    if (hcg_en)
    {
        pstCtrlPara->u32CurrSnsGain = (IMX485RegToGain[gain_val]*166)>>6;
    }

    Sensor_Write(pstCci, 0x3034, hcg_en);
    Sensor_Write(pstCci, 0x3084, (gain_val&0xff));
    Sensor_Write(pstCci, 0x3085, ((gain_val>>8)&0x7));
...
#define SENSOR_IMX485_MIPI    "imx485_mipi"
#ifdef OS_RTT
struct isp_sensor_if imx485_mipi_sensor_if __attribute__((section(".sensor_drv"))) = {
#else
struct isp_sensor_if imx485_mipi_sensor_if = {
#endif

    .name = SENSOR_IMX485_MIPI,
...
struct isp_sensor_if *NAME(Sensor_Create)()
{
    ...
    memcpy(s_if, &imx485_mipi_sensor_if, sizeof(struct isp_sensor_if));
