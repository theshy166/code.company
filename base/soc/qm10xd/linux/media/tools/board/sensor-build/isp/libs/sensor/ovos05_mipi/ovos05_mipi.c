...
static int SetGain(void* pvObj, FH_UINT32 again, FH_UINT8 fNo) {
    ...
    if (again <= 0x80) {  // 1x
        ...
        Reg350a = 0x04;
        Reg350b = 0x00;
    } else if (again <= 0x7ff) {  // 1X~16X a_gain
...
#ifdef OS_RTT
struct isp_sensor_if ovos05_mipi_sensor_if __attribute__((section(".sensor_drv"))) = {
#else
...
struct isp_sensor_if *NAME(Sensor_Create)()
{
    ...
    memcpy(s_if, &ovos05_mipi_sensor_if, sizeof(struct isp_sensor_if));
