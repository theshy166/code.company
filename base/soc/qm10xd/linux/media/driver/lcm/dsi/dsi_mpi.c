...
static int32_t _dsi_init_param_cfg(dsi_init_in_t *inParam)
{
    ...
    dsiContext->vouClkIn.clkInfo.mode = 1/*dsiInitIn->timing.clk.vou.mode*/;
    dsiContext->vouClkIn.clkInfo.cfg.sel = 0x3/*dsiInitIn->timing.clk.vou.sel*/;
    dsiContext->vouClkIn.clkInfo.cfg.div = 0/*dsiInitIn->timing.clk.vou.div*/;
...
int32_t dsi_dcs_write(uint32_t hostId, uint8_t *param, uint16_t param_length)
{
    int32_t rtn = RTN_DSI_SUCCESS;
...
int32_t dsi_dcs_read(uint32_t hostId, uint8_t data_type, uint8_t command, uint8_t bytes_to_read, uint8_t *read_buffer)
{
    int32_t rtn = RTN_DSI_SUCCESS;
...
int32_t dsi_dcs_read_4_identify(uint32_t hostId, uint8_t data_type, uint8_t command, uint8_t bytes_to_read, uint8_t *read_buffer)
{
    int32_t rtn = RTN_DSI_SUCCESS;

    rtn = dsi_dcs_rd_cmd_4_identify(hostId, data_type, command, bytes_to_read, read_buffer);
...
int32_t dsi_push_table(host_core_t hostId, dsi_setting_tab_t *table, uint32_t count)
{
    ...
    for(i = 0; i < count; i++) {
        ...
        switch (di) {
            ...
            {
                dsi_generic_write(hostId, &table[i].para_list[0], table[i].count);
            }
            break;
            ...
            {
                dsi_dcs_write(hostId, &table[i].para_list[0], table[i].count);
            }
            break;
...
int32_t dsi_init(dsi_init_in_t *inParam, dsi_init_out_t *outParam)
{
    ...
    rtn = vou_dhd_force_update(dsiContext->dsiInitIn.vou_base_addr, inParam->hostId, 1);
