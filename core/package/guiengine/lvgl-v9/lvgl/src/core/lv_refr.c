...
static void _allocate_scroll_buf(lv_obj_t * obj) {
    ...
    unsigned long phy_buffer = 0;
    void * virt_buffer = NULL;

    qme_memory_alloc(size, &phy_buffer, &virt_buffer);
    if (virt_buffer) {
        lv_draw_buf_init(&obj->scroll_buf, w, h, cf, stride, virt_buffer, size);
        obj->scroll_buf.phy_addr = phy_buffer;
    } else {
        LV_LOG_ERROR("size=%d alloacte buf failed!\n", size);
    }
}
...
void lv_obj_redraw(lv_layer_t * layer, lv_obj_t * obj)
{
    if(lv_obj_has_flag(obj, LV_OBJ_FLAG_WIDGET_2)) {
        if(lv_obj_has_flag(obj, LV_OBJ_FLAG_USER_1)) {
            ...
            if (!obj->scroll_buf.unaligned_data) {
                _allocate_scroll_buf(obj);
            } else {
                uint32_t w = lv_obj_get_width(obj);
                uint32_t h = lv_obj_get_height(obj);
                if (w != obj->scroll_buf.header.w || h != obj->scroll_buf.header.h) {
                    qme_memory_free(obj->scroll_buf.phy_addr, obj->scroll_buf.unaligned_data);
                    lv_memzero(&obj->scroll_buf, sizeof(lv_draw_buf_t));
                    _allocate_scroll_buf(obj);
                }
            }
            ...
            if (obj->scroll_buf.unaligned_data) {
                ...
                lv_layer_t * layer_scroll = lv_draw_layer_create(layer, LV_COLOR_FORMAT_ARGB8888, &obj_coords);
                layer_scroll->draw_buf = &obj->scroll_buf;
                lv_obj_send_event(obj, LV_EVENT_DRAW_MAIN_BEGIN, layer_scroll);
                lv_obj_send_event(obj, LV_EVENT_DRAW_MAIN, layer_scroll);
                lv_obj_send_event(obj, LV_EVENT_DRAW_MAIN_END, layer_scroll);
                lv_obj_move_children_by(obj, -obj->spec_attr->scroll.x, -obj->spec_attr->scroll.y, true);
                uint32_t child_cnt = lv_obj_get_child_count(obj);
                uint32_t i;
                for(i = 0; i < child_cnt; i++) {
                    lv_obj_t * child = obj->spec_attr->children[i];
                    refr_obj(layer_scroll, child);
                }
                ...
                img_draw_dsc.src = layer_scroll;
                img_draw_dsc.original_area.x1 = obj_coords.x1 + obj->spec_attr->scroll.x;
                img_draw_dsc.original_area.y1 = obj_coords.y1 + obj->spec_attr->scroll.y;
                img_draw_dsc.original_area.x2 = obj_coords.x2 + obj->spec_attr->scroll.x;
                img_draw_dsc.original_area.y2 = obj_coords.y2 + obj->spec_attr->scroll.y;
                lv_draw_layer(layer, &img_draw_dsc, &obj_coords);
...
static void refr_area(const lv_area_t * area_p)
{
    ...
    int32_t aligned_x = ALIGN_BACK(area_p->x1, 4);
    int32_t aligned_y = ALIGN_BACK(area_p->y1, 2);
    int32_t aligned_w = ALIGN_UP((w+area_p->x1-aligned_x), 4);
    int32_t aligned_h = ALIGN_UP((h+area_p->y1-aligned_y), 1);
    ...
    int32_t max_row = get_max_row(disp_refr, aligned_w, aligned_h);
    int32_t row;
    int32_t row_last = 0;
    lv_area_t sub_area;

    if (max_row < aligned_h)
        max_row = ALIGN_BACK(max_row, 2);
    
    for(row = area_p->y1; ALIGN_BACK(row, 2) + max_row - 1 <= y2; row = ALIGN_BACK(row, 2) + max_row) {
        /*Calc. the next y coordinates of draw_buf*/
        sub_area.x1 = aligned_x;
        sub_area.x2 = sub_area.x1 + aligned_w - 1;
        sub_area.y1 = ALIGN_BACK(row, 2);
        sub_area.y2 = sub_area.y1 + max_row - 1;
        layer->draw_buf = disp_refr->buf_act;
