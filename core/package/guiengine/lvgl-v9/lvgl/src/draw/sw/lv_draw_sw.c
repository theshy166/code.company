...
#ifndef XOS_RTT_LVRENDER_STACK_SIZE
#define XOS_RTT_LVRENDER_STACK_SIZE 	8192
#endif
...
#if LV_USE_OS
        lv_thread_init(&draw_sw_unit->thread, LV_THREAD_PRIO_HIGH, render_thread_cb, XOS_RTT_LVRENDER_STACK_SIZE /*8 * 1024*/, draw_sw_unit);
#endif
    }
...
static int32_t dispatch(lv_draw_unit_t * draw_unit, lv_layer_t * layer)
{
    ...
    // int new_x1 = ALIGN_TO_4(t->clip_area.x1);
    // t->clip_area.x2 = new_x1 - t->clip_area.x1 + t->clip_area.x2;
    // t->clip_area.x1 = new_x1;

// printf("layer=%p, layer->buf_area:%d,%d,wh=%d,%d. t->clip_area=%d,%d.wh=%d,%d\n", layer, layer->buf_area.x1, layer->buf_area.y1, lv_area_get_width(&(layer->buf_area)), lv_area_get_height(&(layer->buf_area)) ,t->clip_area.x1, t->clip_area.y1, lv_area_get_width(&t->clip_area), lv_area_get_height(&t->clip_area));
#if LV_USE_OS
