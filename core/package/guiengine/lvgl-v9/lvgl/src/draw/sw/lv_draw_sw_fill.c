...
void lv_draw_sw_fill(lv_draw_unit_t * draw_unit, const lv_draw_fill_dsc_t * dsc, const lv_area_t * coords)
{
    ...
    if(rout > 0) {
        ...
        if (mask_buf == NULL) {
            LV_LOG_WARN("Can't malloc mask_buf clipped_w=%d rout=%d", clipped_w, rout);
            LV_PROFILER_END;
            return;
        }
    ...
    if (grad_dir == LV_GRAD_DIR_NONE) {
        int32_t middle_start, middle_end;
        lv_area_t mask_area = blend_area;
        blend_dsc.mask_area = &mask_area;

        for(h = 0; h < rout; h++) {
            int32_t top_y = bg_coords.y1 + h;

            if(top_y < clipped_coords.y1) continue;

            if(top_y > clipped_coords.y2) break;

            lv_memset(mask_buf + h * clipped_w, opa, clipped_w);
            blend_dsc.mask_res = lv_draw_sw_mask_apply(mask_list, mask_buf + h * clipped_w, blend_area.x1, top_y, clipped_w);
            if (blend_dsc.mask_res == LV_DRAW_SW_MASK_RES_TRANSP)
                LV_LOG_WARN("mask_res is transp x1=%d top_y=%d clipped_w=%d", blend_area.x1, top_y, clipped_w);
        }

        blend_dsc.mask_res = LV_DRAW_SW_MASK_RES_CHANGED;
        middle_start = bg_coords.y1 + h;
        blend_area.y1 = clipped_coords.y1;
        blend_area.y2 = bg_coords.y1 + h - 1;
        mask_area.y1 = bg_coords.y1;
        mask_area.y2 = bg_coords.y1 + rout - 1;
        lv_draw_sw_blend(draw_unit, &blend_dsc);

        for(h = 0; h < rout; h++) {
            int32_t bottom_y = bg_coords.y2 - h;

            if(bottom_y > clipped_coords.y2) continue;

            if(bottom_y < clipped_coords.y1) break;

            lv_memset(mask_buf + (rout - h - 1) * clipped_w, opa, clipped_w);
            blend_dsc.mask_res = lv_draw_sw_mask_apply(mask_list, mask_buf + (rout - h - 1) * clipped_w, blend_area.x1, bottom_y, clipped_w);
            if (blend_dsc.mask_res == LV_DRAW_SW_MASK_RES_TRANSP)
                LV_LOG_WARN("mask_res is transp x1=%d bottom_y=%d clipped_w=%d", blend_area.x1, bottom_y, clipped_w);
        }

        blend_dsc.mask_res = LV_DRAW_SW_MASK_RES_CHANGED;
        middle_end = bg_coords.y2 - h;
        blend_area.y1 = bg_coords.y2 - h + 1;
        blend_area.y2 = clipped_coords.y2;
        mask_area.y1 = bg_coords.y2 - rout + 1;
        mask_area.y2 = bg_coords.y2;
        lv_draw_sw_blend(draw_unit, &blend_dsc);

        blend_area.y1 = middle_start;
        blend_area.y2 = middle_end;
        blend_dsc.opa = opa;
