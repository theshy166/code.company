...
typedef struct {
    ...
    // added by QM
    const void * dest_buf_origin;
    lv_area_t blend_area;
    lv_area_t dest_layer_area;
...
typedef struct {
    ...
    int32_t mask_stride;
    const void * src_buf;
    const void * src_buf_origin;
    int32_t src_stride;
    int32_t src_size;
    lv_color_format_t src_color_format;
    lv_opa_t opa;
    lv_blend_mode_t blend_mode;
    lv_area_t *src_area;//buf original size
    //====qm===
    lv_area_t blend_area;//op rect
    lv_area_t *dest_layer_area;//buf original size
    unsigned long src_phy_addr;
    unsigned long dest_phy_addr;
    //====end qm===
