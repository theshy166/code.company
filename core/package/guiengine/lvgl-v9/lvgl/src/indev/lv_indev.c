...
#ifdef LV_CONFIG_LONG_PRESS_TIME
#define LV_INDEV_DEF_LONG_PRESS_TIME      LV_CONFIG_LONG_PRESS_TIME
#else
#define LV_INDEV_DEF_LONG_PRESS_TIME      5000
#endif
...
static void indev_pointer_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    if(disp->rotation == LV_DISPLAY_ROTATION_180) {
        ...
        data->point.y = disp->hor_res - tmp - 1;
    } else if(disp->rotation == LV_DISPLAY_ROTATION_270) {
...
static void indev_keypad_proc(lv_indev_t * i, lv_indev_data_t * data)
{
    ...
    /*Pressing*/
        ...
        //LV_LOG_USER("%d key is pressed", data->key);
        if(data->key == LV_KEY_F3 && lv_tick_elaps(i->pr_timestamp) > LV_POWERKEY_TIMEOUT && i->long_pr_sent == 0) {
            i->long_pr_sent = 1;
            indev_hook_handler(i, data);
            if(lv_display_send_event(indev_act->disp, LV_EVENT_SCREEN_UNLOAD_START, NULL) == LV_RESULT_INVALID) return;
        }
        ...
        if(i->long_pr_sent == 0 && lv_tick_elaps(i->pr_timestamp) > i->long_press_time) {
            i->long_pr_sent = 1;
            if(data->key == LV_KEY_ENTER || data->key == LV_KEY_RESET) {
