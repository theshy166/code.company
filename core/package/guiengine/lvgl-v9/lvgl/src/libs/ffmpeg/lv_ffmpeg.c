...
static ffmpeg_progress_callback s_progress_cb;
static int s_last_play_time = 0;
static int s_cached_duration = 0;
/**********************
 *      MACROS
 **********************/
                                ...
                                struct ffmpeg_context_s * ffmpeg_ctx)
{
    ...
    while(ret >= 0) {
        ...
        if(dec->codec->type == AVMEDIA_TYPE_VIDEO) {
            ...
            AVRational timebase = ffmpeg_ctx->video_stream->time_base;//ffmpeg_ctx->frame->time_base;
            double playTime = (double)ffmpeg_ctx->frame->best_effort_timestamp * av_q2d(timebase);
            // LV_LOG_USER("playTime=%f, last_play_time=%f", playTime, s_last_play_time);
            if (s_progress_cb != NULL) {
                if (((playTime > 0 && playTime < 0.1) || (playTime > s_cached_duration - 1) || playTime > (s_last_play_time + 0.1))) {
                    s_last_play_time = playTime;
                    s_progress_cb(playTime);
                }
            }
        }
...
//other function
int64_t los_ffmpeg_player_get_duration(lv_obj_t *obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_ffmpeg_player_t * player = (lv_ffmpeg_player_t *)obj;
    s_cached_duration = player->ffmpeg_ctx->fmt_ctx->duration/AV_TIME_BASE;
    return s_cached_duration;
}

int64_t los_ffmpeg_player_get_current(lv_obj_t *obj)
{
    LV_ASSERT_OBJ(obj, MY_CLASS);
    lv_ffmpeg_player_t * player = (lv_ffmpeg_player_t *)obj;
    return player->ffmpeg_ctx->frame->pts * av_q2d(player->ffmpeg_ctx->frame->time_base);
}

void los_ffmpeg_player_set_callback(ffmpeg_progress_callback pcb)
{
    s_progress_cb = pcb;
}
