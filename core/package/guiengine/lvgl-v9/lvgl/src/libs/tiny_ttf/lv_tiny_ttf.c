...
lv_mutex_t ttf_stream_op_mutex;
#endif
...
void lv_tiny_ttf_init(void)
{
    ...
    tiny_ttf_cache = lv_cache_create(&lv_cache_class_lru_rb_count, sizeof(tiny_ttf_cache_data_t), 1024, ops);

#if LV_TINY_TTF_FILE_SUPPORT != 0
    lv_mutex_init(&ttf_stream_op_mutex);
#endif
}
...
static void ttf_cb_stream_read(ttf_cb_stream_t * stream, void * data, size_t to_read)
{
    ...
    lv_mutex_unlock(&ttf_stream_op_mutex);
}
static void ttf_cb_stream_seek(ttf_cb_stream_t * stream, size_t position)
{
    lv_mutex_lock(&ttf_stream_op_mutex);
    if(stream->file != NULL) {
