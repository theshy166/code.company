...
#include <string.h>
#include <errno.h>
#include "pipeline/vdec_display/qua_vdec_display_pipeline.h"
...
int main(int argc, char* argv[]) {
    ...
    qua_vdec_display_pipeline_attr_t attr;
    ...
    attr.x = 0;
    attr.y = 0;
    attr.display_width = 1024;
    attr.display_height = 600;
    QUA_U32 buf_len = width * height;
    QUA_U8 *pu8_buf = malloc(buf_len);

    qua_vdec_display_pipeline_ops_t pipeline_ops;
    ret = qua_vdec_display_pipeline_init(&pipeline_ops);
    if (ret != QUA_SUCCESS)
        return -1;
    qua_vdec_display_pipeline_t pipeline = NULL;
