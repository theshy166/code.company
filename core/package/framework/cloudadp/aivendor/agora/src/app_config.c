...
void app_print_config(app_config_t *config) {
  ...
  LOGS("  sdk-log-dir             : %s", config->p_sdk_log_dir != NULL ? config->p_sdk_log_dir : "NULL");
  ...
  LOGS("  pcm_channel_num         : %u", config->pcm_channel_num);
  ...
  LOGS("  send resolution         : %d", config->resolution);
  LOGS("  video_exist             : %d", config->video_exist);
  LOGS("  audio_volume            : %d", config->audio_volume);
	LOGS("---------------app config show end-----------------------");
}

int app_parse_args(int argc, char **argv, app_config_t *config)
{
  const char *av_short_option = "hi:t:c:u:v:a:C:f:S:s:r:n:A:gmRl:dD:L:x:e:M:";
  ...
  const struct option av_long_option[] = { { "help", 0, NULL, 'h' },
                                           ...
                                           { "resultion", 1, NULL, 'x'},
                                           { "video_exist", 1, NULL, 'e'},
                                           { "audio_volume", 1, NULL, 'M'}, 
                                           { 0, 0, 0, 0 } };
  ...
  while (1) {
    ...
    case 'x':
      ...
      break;
    case 'e':
      config->video_exist = atoi(optarg);
      break;
    case 'M':
      config->audio_volume = atoi(optarg);
      break;
    case 'c':
    ...
    case 0:
      ...
      break;
    default:
      LOGS("Unknown cmd param %s. ch=%c", av_long_option[optidx].name, ch);
      return -1;
    }
  }
  optind = saved_optind; // 恢复optind
