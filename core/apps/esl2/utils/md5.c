...
char * MD5Char(const char *str)
{
    ...
    MD5 digest;
    MD5Init(&context);
    MD5Update(&context, (unsigned char*)str, (UINT)strlen(str));
    MD5Final(digest, &context);
    ...
    // 动态分配内存来存储结果
    char *result = (char*)malloc(strlen(mdstr) + 1);
    if (result != NULL) {
        strcpy(result, mdstr);
    }
    QM_ESL2_LOG("result: %p\n", (void*)result);
    return result;
}
...
void MD5String(MD5 digest, MD5_STR mdstr)
{
    ...
    for (i = 0; i < MD5_VALUE_LEN; i++)
        j += snprintf(mdstr + j, MD5_STRING_LEN - j, "%02X", digest[i]);
}
