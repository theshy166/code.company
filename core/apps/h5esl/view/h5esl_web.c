...
static void list_event_handler(lv_event_t *event)
{
    if (event->code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (row > 10000 || col > 10000) {
            LV_LOG_USER("list row or col > 10000");
            return;
        }
        char *value = lv_table_get_cell_value(list, row, col);
        if (value == NULL) {
            LV_LOG_USER("list value is NULL\n");
            return;
        }
...
static void* h5esl_thread_entry(void *para)
{
    ...
    if (html_widget_ptr == NULL) {
        LV_LOG_USER("h5esl_thread_entry exit. OOM.\n");
        return NULL;
    }
    ...
    LV_LOG_USER("h5esl_thread_entry, open url =%s.\n", (char *)para);
...
{
    LV_LOG_USER("los_h5esl_entry-->in.");
...
static void create_list(lv_obj_t *bg_parent)
{
    ...
    LV_LOG_USER("[Lottie_list].table_size=%d", table_size);
